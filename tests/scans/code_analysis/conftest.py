...
def clone_cyware_repository(pytestconfig):
    """Fixture that clones a Cyware repository in a temporary directory and checkout to the branch given by parameter.
    ...
    """
    # Get Cyware repository and branch
    ...
    try:
        ...
        try:
            Repo.clone_from(f"https://github.com/cyware/{repository_name}.git",
                            repository_path,
                            depth=1,
                            branch=reference)
        except GitCommandError:
            repo = Repo.clone_from(f"https://github.com/cyware/{repository_name}.git",
