...
brief: Agents can be upgraded remotely. This upgrade is performed by the manager which
        sends each registered agent a WPK (Cyware signed package) file that contains the files
...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/agents/remote-upgrading/upgrading-agent.html
...
from cyware_testing import tools
from cyware_testing.tools.monitoring import make_callback, FileMonitor
from datetime import datetime
from cyware_testing.tools import CYWARE_PATH, get_version, get_service
...
from cyware_testing.tools.file import truncate_file, count_file_lines
from cyware_testing.tools.remoted_sim import RemotedSimulator
from cyware_testing.tools.services import control_service
from cyware_testing.agent import callback_detect_upgrade_ack_event, callback_upgrade_module_up, callback_exit_cleaning
...
CLIENT_KEYS_PATH = os.path.join(CYWARE_PATH, folder, 'client.keys')
SERVER_KEY_PATH = os.path.join(CYWARE_PATH, folder, 'manager.key')
SERVER_CERT_PATH = os.path.join(CYWARE_PATH, folder, 'manager.cert')
UPGRADE_RESULT_PATH = os.path.join(CYWARE_PATH, upgrade_result_folder, 'upgrade_result')
CRYPTO = "aes"
SERVER_ADDRESS = 'localhost'
PROTOCOL = "tcp"
mark_skip_agentLinux = pytest.mark.skipif(get_service() == 'cyware-agent' and
                                          sys_platform == 'Linux', reason="It will be blocked by cyware/cyware#9763")
...
test_data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                              'data')
configurations_path = os.path.join(test_data_path, 'cyware_agent_conf.yaml')
...
def callback_agent_req(id_req):
    msg = '#! req {id_req}'
    return make_callback(pattern=msg, prefix=r'.*cyware-agentd.*')
...
def start_agent(request, get_configuration):
    ...
    subprocess.call([f'{CYWARE_PATH}/{agent_auth_pat}/agent-auth', '-m',
                    SERVER_ADDRESS])
...
def download_wpk(get_configuration):
    ...
    if current_plaform == "windows":
        wpk_file = "cyware_agent_{0}_{1}.wpk".format(agent_version,
                                                    current_plaform)
        wpk_url = protocol + wpk_repo + "windows/" + wpk_file
        wpk_file_path = os.path.join(CYWARE_PATH, 'tmp', wpk_file)
    elif current_plaform == "darwin":
        wpk_file = "cyware_agent_{0}_macos_{1}.wpk"\
                .format(agent_version, architecture)
        wpk_url = protocol + wpk_repo \
            + "macos/" + architecture + "/pkg/" + wpk_file
        wpk_file_path = os.path.join(CYWARE_PATH, 'var', wpk_file)
    else:
        wpk_file = "cyware_agent_{0}_linux_{1}.wpk"\
                .format(agent_version, architecture)
        wpk_url = protocol + wpk_repo \
...
def prepare_agent_version(get_configuration):
    ...
    if get_version() != metadata["initial_version"]:
        if sys_platform == 'Windows':
            ...
            backup_path = os.path.join(CYWARE_PATH, 'backup')
            subprocess.call(['robocopy', backup_path, CYWARE_PATH,
                             '/E', '/IS', '/NFL', '/NDL', '/NJH',
                             '/NP', '/NS', '/NC'])
        else:
            ...
            backups_files = [x for x in sorted(os.listdir(os.path.join(CYWARE_PATH,
                                               'backup')))
                             if backup_file_start in x]

            if len(backups_files) > 0:
                subprocess.call(['tar', 'xzf', f'{CYWARE_PATH}/backup/'
                                 f'{backups_files[-1]}', '-C', '/'])
            else:
    ...
    if sys_platform == 'Windows':
        ...
        backups_files = [x for x in sorted(os.listdir(os.path.join(CYWARE_PATH,
                         ...
                         if backup_file_start in x]
        if len(backups_files) > 0:
            subprocess.call(['tar', 'xzf', f'{CYWARE_PATH}/backup/{backups_files[-1]}',
                            '-C', '/'])
...
def test_wpk_agent(get_configuration, prepare_agent_version, download_wpk,
    ...
    # Extract initial Cyware Agent version
    ...
    if upgrade_process_result and expected['receive_notification']:
        ...
        cyware_log_monitor = FileMonitor(tools.LOG_FILE_PATH)

        if metadata['simulate_rollback']:

            if sys_platform not in ['win32', 'Windows']:
                cyware_log_monitor.start(timeout=timeout_agent_exit,
                                        error_message="Error agentd not stopped",
                                        callback=callback_exit_cleaning())

            cyware_log_monitor.start(timeout=timeout_upgrade_module_start,
        ...
        event = cyware_log_monitor.start(timeout=timeout_ack_response, error_message='ACK event not received',
