...
from cyware_testing.tools import CYWARE_PATH, LOG_FILE_PATH, UPGRADE_PATH, get_version
from cyware_testing.tools.configuration import load_cyware_configurations
from cyware_testing.tools.agent_simulator import Sender, Injector
...
from cyware_testing.tools.sockets import CywareSocket

pytestmark = [pytest.mark.linux, pytest.mark.tier(level=0), pytest.mark.server]

UPGRADE_SOCKET = os.path.join(CYWARE_PATH, 'queue', 'tasks', 'upgrade')
TASK_SOCKET = os.path.join(CYWARE_PATH, 'queue', 'tasks', 'task')
SERVER_ADDRESS = 'localhost'
WPK_REPOSITORY_4x = global_parameters.wpk_package_path[0]
WPK_REPOSITORY_3x = 'packages.cyware.khulnasoft.com/wpk/'
CRYPTO = "aes"
CHUNK_SIZE = 16384
TASK_TIMEOUT = '15m'
...
upgrade_socket = CywareSocket(UPGRADE_SOCKET)
task_socket = CywareSocket(TASK_SOCKET)
...
def set_debug_mode():
    ...
    debug_line = 'cyware_modules.debug=2\n'
...
def get_sha_list(metadata):
    ...
    wpk_file = "cyware_agent_{0}_linux_{1}.wpk".format(agent_version, architecture)
...
@pytest.mark.skip(reason="Blocked by issue cyware-qa#2203, when is fixed we can enable this test again")
