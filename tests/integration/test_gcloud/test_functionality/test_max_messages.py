...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/gcp-pubsub.html#max-messages
...
from cyware_testing.gcloud import callback_detect_start_fetching_logs, callback_received_messages_number
...
if get_service() == 'cyware-manager':
    daemons_handler_configuration = {'daemons': ['cyware-modulesd', 'cyware-analysisd']}
else:
...
@pytest.mark.xfail(reason='Unstable, further information in cyware/cyware#17245')
def test_max_messages(get_configuration, configure_environment, reset_ossec_log, publish_messages,
    ...
    cyware_log_monitor.start(timeout=global_parameters.default_timeout + time_interval,
                            callback=callback_detect_start_fetching_logs,
                            error_message='Did not receive expected '
                                          '"Starting fetching of logs" event')

    numbers_pulled = cyware_log_monitor.start(timeout=pull_messages_timeout,
