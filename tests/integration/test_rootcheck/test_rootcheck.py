...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/capabilities/policy-monitoring/rootcheck
...
from cyware_testing.tools.agent_simulator import Sender, Injector, create_agents
...
CYWARE_DB_DIR = os.path.join(CYWARE_PATH, 'queue', 'db')
...
def retrieve_rootcheck_rows(agent_id):
    agent_db_path = os.path.join(CYWARE_DB_DIR, f'{agent_id}.db')
...
def test_rootcheck(get_configuration, configure_environment, restart_service,
    ...
    input_description: Different test cases are contained in an external YAML file (cyware_manager_conf.yaml)
    ...
    expected_output:
        ...
        - Cyware DB returned an error trying to delete the agent
    ...
    # Service needs to be stopped otherwise db lock will be held by Cyware db
    ...
    if check_delete:
        ...
        for agent in agents:
            ...
            assert response.startswith(b'ok'), "Cyware DB returned an error " \
