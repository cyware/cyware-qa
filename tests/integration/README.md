...
Cyware - Quality assurance automation templates
...
check [Cyware documentation](https://documentation.cyware.khulnasoft.com/3.11/installation-guide/index.html)._
...
```shell script
# Enable debug 2
echo 'syscheck.debug=2' >> $cyware_path/etc/local_internal_options.conf
...
echo 'monitord.rotate_log=0' >> $cyware_path/etc/local_internal_options.conf

# Restart Cyware
/var/ossec/bin/cyware-control restart
```
...
# Restart Cyware
/Library/Ossec/bin/cyware-control restart
```

-----------

Finally, copy your `cyware-qa` repository within your testing environment and you are set.
...
Our newest integration tests are located in `cyware-qa/tests/integration/`. They are organized by capabilities:
...
- **Services**: from controlling Cyware services, daemons and socket to common processes.
...
- `--wpk_version`: Specify the WPK package version used to upgrade on WPK tests. (ex. --wpk_version=v4.2.0). Note: This
  ...
  the [repository](packages-dev.cyware.khulnasoft.com/trash/wpk/).
- `--wpk_package_path`: Specify the WPK package path used to upgrade on WPK tests. (ex. --wpk_package_path='packages-dev.cyware.khulnasoft.com/trash/wpk/').
...
rootdir: /vagrant/cyware-qa/test_cyware, inifile: pytest.ini
collected 12 items
