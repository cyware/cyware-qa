...
from cyware_testing.tools import CYWARE_LOGS_PATH, LOG_FILE_PATH, CYWARE_PATH
from cyware_testing.tools.file import truncate_file
from cyware_testing.tools.monitoring import ManInTheMiddle, QueueMonitor
...
def generate_analysisd_yaml(n_events, modify_events):
    ...
    def remove_logs():
        for root, _, files in os.walk(CYWARE_LOGS_PATH):
            for file in files:
    ...
    control_service('start', daemon='cyware-remoted', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyware-remoted')
...
def kill_daemons():
    for daemon in ['cyware-remoted', 'cyware-analysisd', 'cyware-db']:
