...
import yaml
from cyware_testing import logger
from cyware_testing.analysis import callback_analysisd_agent_id, callback_analysisd_event
from cyware_testing.fim import REGULAR, create_file, modify_file, delete_file, detect_initial_scan
from cyware_testing.tools import CYWARE_LOGS_PATH, LOG_FILE_PATH, CYWARE_PATH, CYWARE_CONF, PREFIX
from cyware_testing.tools.configuration import generate_syscheck_config
from cyware_testing.tools.file import truncate_file
from cyware_testing.tools.monitoring import ManInTheMiddle, QueueMonitor, FileMonitor
from cyware_testing.tools.services import control_service, check_daemon_status, delete_sockets

alerts_json = os.path.join(CYWARE_LOGS_PATH, 'alerts', 'alerts.json')
...
def set_syscheck_config():
    original_conf = open(CYWARE_CONF, 'r').readlines()
    directory = 0

    with open(CYWARE_CONF, 'w') as new_conf:
        syscheck_flag = False
...
def set_syscheck_backup(original_conf):
    with open(CYWARE_CONF, 'w') as o_conf:
...
def generate_analysisd_yaml(n_events, modify_events):
    ...
    def remove_logs():
        for root, dirs, files in os.walk(CYWARE_LOGS_PATH):
            for file in files:
    ...
    control_service('start', daemon='cyware-db', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyware-db')

    control_service('start', daemon='cyware-analysisd', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyware-analysisd')
    ...
    control_service('start', daemon='cyware-syscheckd', debug_mode=True)
    check_daemon_status(running_condition=True, target_daemon='cyware-syscheckd')
...
def kill_daemons():
    for daemon in ['cyware-analysisd', 'cyware-db', 'cyware-syscheckd']:
