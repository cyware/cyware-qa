...
from cyware_testing.tools import (ALERT_FILE_PATH, LOG_FILE_PATH,
                                 CYWARE_UNIX_USER, CYWARE_UNIX_GROUP,
...
def configure_local_rules(get_configuration, request):
    """Configure a custom rule in local_rules.xml for testing. Restart Cyware is needed for applying the
    configuration. """
...
def configure_custom_rules(request):
    ...
    Restarting cyware-analysisd is required to apply this changes.
    """
    ...
    shutil.chown(target_rule, CYWARE_UNIX_USER, CYWARE_UNIX_GROUP)
