...
brief: The 'cyware-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'cyware-analysisd' daemon handles correctly
...
from cyware_testing.analysis import callback_fim_error
...
monitored_sockets_params = [('cyware-analysisd', mitm_analysisd, True)]
...
def test_error_messages(configure_sockets_environment, connect_to_sockets_module, wait_for_analysisd_startup,
                        test_case: list):
    '''
    description: Check if when the 'cyware-analysisd' daemon socket receives a message with an invalid event,
                 it generates the corresponding error that sends to the 'cyware-db' daemon socket.
