...
brief: The 'cyware-analysisd' daemon refreshes the queue socket file every time the configuration test is executed
       Specifically, this test will check if after running the configuration test of 'cyware-analysisd' the properties
...
from cyware_testing.tools import ANALYSISD_BINARY_PATH, ANALYSISD_QUEUE_SOCKET_PATH
...
def socket_file_properties():
    """Get the inode and modification time values of the 'queue' socket of 'cyware-analysisd'"""
...
def run_analysisd_test_config():
    """Run the daemon configuration test mode of 'cyware-analysisd'"""
...
def test_queue_socket_properties(restart_analysisd, before_socket_properties, run_analysisd_test_config,
    ...
    '''
    description: Check if after running the configuration test of 'cyware-analysisd' the properties
                 of the queue socket file are changed.
    ...
    parameters:
        ...
        - run_analysisd_test_config:
            type: fixture
            brief: Run the daemon configuration test mode of 'cyware-analysisd'
        - after_socket_properties:
            type: fixture
    ...
    assertions:
        - Verify that the Inode value of the socket file does not change its value after running the
          configuration test of 'cyware-analysisd'
    ...
    input_description: The test gets the current properties of the socket file and some parameters
                       to run the daemon configuration test of 'cyware-analysisd'.
