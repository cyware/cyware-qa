...
from cyware_testing import ARCHIVES_LOG_PATH
from cyware_testing.modules.analysisd import event_monitor as evm
from cyware_testing.tools import file
from cyware_testing.modules.analysisd import QUEUE_EVENTS_SIZE, ANALYSISD_ONE_THREAD_CONFIG
from cyware_testing.scripts.syslog_simulator import DEFAULT_MESSAGE_SIZE
from cyware_testing.tools.run_simulator import syslog_simulator
...
local_internal_options = {'cyware_modules.debug': '2', 'monitord.rotate_log': '0', 'analysisd.state_interval': '1'}
...
t4_local_internal_options = {'cyware_modules.debug': '2', 'monitord.rotate_log': '0', 'analysisd.state_interval': '1'}
...
def test_limitation(configuration, metadata, load_cyware_basic_configuration, set_cyware_configuration,
                    configure_local_internal_options_function, truncate_monitored_files, restart_cyware_daemon_function):
    """
    ...
    # Check that cyware-manager is processing syslog events
    ...
    # Check that the cyware-manager is receiving events but it is not processing them due to the limitation
...
def test_queueing_events_after_limitation(configuration, metadata, load_cyware_basic_configuration,
                                          set_cyware_configuration, configure_local_internal_options_function,
                                          truncate_monitored_files, restart_cyware_daemon_function):
    """
...
def test_dropping_events_when_queue_is_full(configuration, metadata, load_cyware_basic_configuration,
                                            set_cyware_configuration, configure_local_internal_options_function,
                                            truncate_monitored_files, restart_cyware_daemon_function):
    """
...
def test_event_processing_in_order_single_thread(configuration, metadata, load_cyware_basic_configuration,
                                                 set_cyware_configuration, configure_local_internal_options_function,
                                                 truncate_event_logs, restart_cyware_daemon_function):
    """
    ...
    parameters:
        ...
        - truncate_event_logs:
            type: fixture
            brief: Truncate cyware event logs.
        - restart_cyware_daemon_function:
            type: fixture
            brief: Restart the cyware service.
...
def test_event_processing_in_order_multi_thread(configuration, metadata, load_cyware_basic_configuration,
                                                set_cyware_configuration, configure_local_internal_options_function,
                                                truncate_event_logs, restart_cyware_daemon_function):
    """
    description: Check that events are processed in order according to the position within the queue, and
