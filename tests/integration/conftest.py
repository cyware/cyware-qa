...
from cyware_testing import ALERTS_JSON_PATH, ARCHIVES_JSON_PATH, ARCHIVES_LOG_PATH, global_parameters, logger, mocking
from cyware_testing.db_interface.agent_db import update_os_info
from cyware_testing.db_interface.global_db import get_system, modify_system
from cyware_testing.logcollector import create_file_structure, delete_file_structure
from cyware_testing.tools import (PREFIX, LOG_FILE_PATH, CYWARE_CONF, get_service, ALERT_FILE_PATH,
                                 CYWARE_LOCAL_INTERNAL_OPTIONS, AGENT_CONF, AGENT_INFO_SOCKET_PATH)
from cyware_testing.tools import ALERT_FILE_PATH, LOG_FILE_PATH, CYWARE_CONF, CYWARE_LOCAL_INTERNAL_OPTIONS, get_service
from cyware_testing.tools import (PREFIX, LOG_FILE_PATH, CYWARE_CONF, get_service, ALERT_FILE_PATH,
                                 CYWARE_LOCAL_INTERNAL_OPTIONS)
from cyware_testing.tools.configuration import get_minimal_configuration, get_cyware_conf, write_cyware_conf
from cyware_testing.tools.file import (truncate_file, recursive_directory_creation, remove_file, copy, write_file,
                                      delete_path_recursively)
from cyware_testing.tools.monitoring import FileMonitor, QueueMonitor, SocketController, close_sockets
from cyware_testing.tools.services import check_daemon_status, control_service, delete_dbs
from cyware_testing.tools.time import TimeMachine
import cyware_testing.tools.configuration as conf


if sys.platform == 'win32':
    from cyware_testing.fim import (KEY_WOW64_32KEY, KEY_WOW64_64KEY,
...
###############################
report_files = [LOG_FILE_PATH, CYWARE_CONF, CYWARE_LOCAL_INTERNAL_OPTIONS]
...
def restart_cyware(get_configuration, request):
    # Stop Cyware
...
@pytest.fixture(scope='module')
def restart_cyware_daemon(daemon=None):
    """
    Restart a Cyware daemon
    """
    truncate_file(LOG_FILE_PATH)
...
def restart_cyware_daemon_function(daemon=None):
    """
    Restart a Cyware daemon
    """
    truncate_file(LOG_FILE_PATH)
...
@pytest.fixture(scope='function')
def restart_cyware_function(request):
    ...
    if len(daemons) == 0:
        ...
        # Stop a list daemons in order (as Cyware does)
        daemons.reverse()
        for daemon in daemons:
...
@pytest.fixture(scope='module')
def restart_cyware_module(daemon=None):
    """Restart all Cyware daemons"""
    control_service("restart", daemon=daemon)
    yield
    control_service('stop', daemon=daemon)
...
@pytest.fixture(scope='module')
def restart_cyware_daemon_after_finishing(daemon=None):
    """
    Restart a Cyware daemon
    """
    yield
...
@pytest.fixture(scope='function')
def restart_cyware_daemon_after_finishing_function(daemon=None):
    """
    Restart a Cyware daemon
    """
    yield
...
def restart_analysisd_function():
    """Restart cyware-analysisd daemon before starting a test, and stop it after finishing"""
    control_service('restart', daemon='cyware-analysisd')
    yield
    control_service('stop', daemon='cyware-analysisd')
...
@pytest.fixture(scope='module')
def restart_cyware_alerts(get_configuration, request):
    # Stop Cyware
    control_service('stop')
    ...
    setattr(request.module, 'cyware_alert_monitor', file_monitor)
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        help="number of seconds that any timer will wait until an event is generated. This apply to all tests except"
             ...
             "variable from cyware_testing package"
    )
    parser.addoption(
        "--fim-database-memory",
        action="store_true",
...
def configure_local_internal_options(get_local_internal_options):
    """Configure Cyware local internal options.
    ...
    if backup_options_dict != get_local_internal_options:
        conf.add_cyware_local_internal_options(get_local_internal_options)
...
def configure_environment(get_configuration, request):
    """Configure a custom environment for testing. Restart Cyware is needed for applying the configuration."""

    # Save current configuration
    backup_config = conf.get_cyware_conf()

    # Configuration for testing
    test_config = conf.set_section_cyware_conf(get_configuration.get('sections'))
    ...
    conf.write_cyware_conf(test_config)
    ...
    conf.write_cyware_conf(backup_config)
...
def set_agent_conf(get_configuration):
    ...
    new_config = conf.set_section_cyware_conf(sections, backup_config)
...
def configure_sockets_environment(request):
    ...
    # Stop cyware-service and ensure all daemons are stopped
    control_service('stop')
...
def daemons_handler_impl(request):
    """Helper function to handle Cyware daemons.
    ...
    The  `daemons_handler_configuration` should be a dictionary with the following keys:
        daemons (list, optional): List with every daemon to be used by the module. In case of empty a ValueError
            will be raised
        all_daemons (boolean): Configure to restart all cyware services. Default `False`.
    ...
    try:
        ...
        if 'all_daemons' in daemons_handler_configuration:
            logger.debug(f"Cyware control set to {daemons_handler_configuration['all_daemons']}")
    ...
    try:
        if all_daemons:
            logger.debug('Restarting cyware using cyware-control')
            # Restart daemon instead of starting due to legacy used fixture in the test suite.
            control_service('restart')
        else:
            for daemon in daemons:
    ...
    if all_daemons:
        logger.debug('Stopping cyware using cyware-control')
        control_service('stop')
    else:
        for daemon in daemons:
            logger.debug(f"Stopping {daemon}")
...
@pytest.fixture()
def set_cyware_configuration(configuration):
    ...
    # Configuration for testing
    test_config = conf.set_section_cyware_conf(configuration.get('sections'))
...
def stop_modules_function_after_execution():
    """Stop cyware modules daemon after finishing a test"""
    yield
    control_service('stop')
...
def mock_agent_module():
    """Fixture to create a mocked agent in cyware databases"""
...
def load_cyware_basic_configuration():
    ...
    backup_ossec_configuration = conf.get_cyware_conf()

    # Write new configuration
    conf.write_cyware_conf(minimal_configuration)

    yield

    # Restore the ossec.conf backup
    conf.write_cyware_conf(backup_ossec_configuration)
