...
from cyware_testing.tools import LOG_FILE_PATH, CLIENT_KEYS_PATH, API_LOG_FILE_PATH
from cyware_testing.cyware_db import insert_agent_in_db, clean_agents_from_db
from cyware_testing.tools.file import truncate_file
from cyware_testing.tools.monitoring import FileMonitor, make_callback, AUTHD_DETECTOR_PREFIX
from cyware_testing.tools.configuration import write_cyware_conf, get_cyware_conf, set_section_cyware_conf,\
                                              load_cyware_configurations
from cyware_testing.tools.services import control_service
from cyware_testing.authd import DAEMON_NAME
from cyware_testing.api import get_api_details_dict
from cyware_testing.tools.cyware_manager import remove_agents
from cyware_testing.modules.api import event_monitor as evm
...
def format_configuration(get_current_test_case, request):
    ...
    conf = load_cyware_configurations(temp, test_name)
    os.remove(temp)

    test_config = set_section_cyware_conf(conf[0]['sections'])
...
def override_authd_force_conf(format_configuration):
    """
    Re-writes Cyware configuration file with new configurations from the test case.
    """
    # Save current configuration
    backup_config = get_cyware_conf()

    # Set new configuration
    write_cyware_conf(format_configuration)

    yield

    # Restore previous configuration
    write_cyware_conf(backup_config)
