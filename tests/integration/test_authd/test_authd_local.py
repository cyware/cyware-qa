...
brief: This module verifies the correct behavior of 'cyware-authd' under different messages
...
from cyware_testing.tools import CYWARE_PATH, CYWARE_DB_SOCKET_PATH
...
receiver_sockets_params = [(ls_sock_path, 'AF_UNIX', 'TCP'), (CYWARE_DB_SOCKET_PATH, 'AF_UNIX', 'TCP')]
test_case_ids = [f"{test_case['name'].lower().replace(' ', '-')}" for test_case in message_tests]

# TODO Replace or delete
monitored_sockets_params = [('cyware-db', None, True), ('cyware-authd', None, True)]
...
def test_authd_local_messages(configure_environment, configure_sockets_environment, connect_to_sockets_function,
                              set_up_groups, insert_pre_existent_agents, restart_cyware_daemon_function,
