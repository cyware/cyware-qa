...
brief: These tests will check if the 'cyware-authd' daemon is able to handle secure connections using
       the 'SSL' (Secure Socket Layer) protocol. The 'cyware-authd' daemon can automatically add
       a Cyware agent to a Cyware manager and provide the key to the agent.
...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/registering/host-verification-registration.html
...
from cyware_testing.tools.configuration import set_section_cyware_conf, write_cyware_conf, \
    load_cyware_configurations
from cyware_testing.tools.file import truncate_file
from cyware_testing.tools.monitoring import SocketController, FileMonitor
from cyware_testing.tools.security import CertificateController
...
metadata = [{'sim_option': option, 'verify_host': ssl_verify_host} for ssl_verify_host in SSL_VERIFY_HOSTS for option in
            SIM_OPTIONS]
configurations = load_cyware_configurations(configurations_path, __name__, params=params, metadata=metadata)
# Simulation options
# a. Unverified Host:
# - No certificate
# - Valid Certificate
...
def override_cyware_conf(configuration):
    # Stop Cyware
    control_service('stop', daemon='cyware-authd')
    time.sleep(1)
    check_daemon_status(running_condition=False, target_daemon='cyware-authd')
    truncate_file(LOG_FILE_PATH)

    # Configuration for testing
    test_config = set_section_cyware_conf(configuration.get('sections'))
    # Set new configuration
    write_cyware_conf(test_config)
    # Start Cyware daemons
    time.sleep(1)
    control_service('start', daemon='cyware-authd')
...
def test_authd_ssl_certs(get_configuration, generate_ca_certificate, tear_down):
    '''
    description:
        Checks if the 'cyware-authd' daemon can manage 'SSL' connections with agents
    ...
    assertions:
        - Verify that the agent can only connect to the 'cyware-authd' daemon socket using a valid certificate.
    ...
    option = get_configuration['metadata']['sim_option']
    override_cyware_conf(get_configuration)
