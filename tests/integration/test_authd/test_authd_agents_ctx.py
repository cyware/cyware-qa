...
brief: These tests will check if the 'cyware-authd' daemon correctly handles the enrollment requests
       from agents with pre-existing IP addresses or names. The 'cyware-authd' daemon can automatically
       add a Cyware agent to a Cyware manager and provide the key to the agent. It is used along with
...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/daemons/cyware-authd.html
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/tools/agent_groups.html
...
from cyware_testing.tools.file import truncate_file, remove_file, recursive_directory_creation
from cyware_testing.tools.monitoring import FileMonitor
from cyware_testing.tools.services import control_service, check_daemon_status
from cyware_testing.api import remove_groups, set_up_groups
...
configurations = load_cyware_configurations(configurations_path, __name__, params=None, metadata=None)
CLIENT_KEYS_PATH = os.path.join(CYWARE_PATH, 'etc', 'client.keys')

# Variables
log_monitor_paths = []

ls_sock_path = os.path.join(os.path.join(CYWARE_PATH, 'queue', 'sockets', 'auth'))
receiver_sockets_params = [(("localhost", 1515), 'AF_INET', 'SSL_TLSv1_2'), (ls_sock_path, 'AF_UNIX', 'TCP')]

monitored_sockets_params = [('cyware-modulesd', None, True), ('cyware-db', None, True), ('cyware-authd', None, True)]
...
def clean_diff():
    diff_folder = os.path.join(CYWARE_PATH, 'queue', 'diff')
    try:
...
def clean_rids():
    rids_folder = os.path.join(CYWARE_PATH, 'queue', 'rids')
...
def clean_agents_timestamp():
    timestamp_path = os.path.join(CYWARE_PATH, 'queue', 'agents-timestamp')
...
def check_diff(name, expected, timeout=30):
    diff_path = os.path.join(CYWARE_PATH, 'queue', 'diff', name)
    wait = time.time() + timeout
    while time.time() < wait:
...
def check_rids(id, expected):
    agent_info_path = os.path.join(CYWARE_PATH, 'queue', 'rids', id)
...
def create_rids(id):
    rids_path = os.path.join(CYWARE_PATH, 'queue', 'rids', id)
    try:
...
def create_diff(name):
    SIGID = '533'
    diff_folder = os.path.join(CYWARE_PATH, 'queue', 'diff', name)
    try:
...
def test_ossec_authd_agents_ctx(get_configuration, configure_environment, configure_sockets_environment,
                                     connect_to_sockets_module, restart_cyware, server_type):
    '''
    description:
        Check if when the 'cyware-authd' daemon receives an enrollment request from an agent
    ...
    input_description:
        Different test cases are contained in an external YAML file (cyware_conf.yaml)
        which includes configuration settings for the 'cyware-authd' daemon.

    expected_output:
        - r'Accepting connections on port 1515' (When the 'cyware-authd' daemon is ready to accept enrollments)
    ...
    check_daemon_status(running_condition=False, target_daemon='cyware-authd')
    time.sleep(1)
    clean_logs()
    clean_agents_ctx()
    control_service('start')
    check_daemon_status(running_condition=True, target_daemon='cyware-authd')
