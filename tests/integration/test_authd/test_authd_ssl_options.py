...
brief: These tests will check if the 'SSL' (Secure Socket Layer) protocol-related settings of
       the 'cyware-authd' daemon are working correctly. The 'cyware-authd' daemon can
       automatically add a Cyware agent to a Cyware manager and provide the key
...
from cyware_testing.tools.file import truncate_file, read_yaml
...
def override_cyware_conf(configuration):
    """
    Write a particular Cyware configuration for the test case.
    """
    # Stop Cyware
...
def test_ossec_auth_configurations(get_configuration, configure_environment, configure_sockets_environment):
    '''
    description:
        Checks if the 'SSL' settings of the 'cyware-authd' daemon work correctly by enrolling agents
