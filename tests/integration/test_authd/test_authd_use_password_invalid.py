...
from cyware_testing.modules.authd import event_monitor as evm
from cyware_testing import DEFAULT_AUTHD_PASS_PATH
from cyware_testing.tools.file import write_file, delete_file
...
def test_authd_use_password_invalid(metadata, configuration, truncate_monitored_files,
                                    ...
                                    set_cyware_configuration, tear_down):
    '''
    description:
    ...
    assertions:
        ...
        - cyware-manager.service must not be able to restart.

    input_description:
        ./data/configuration_template/config_authd_use_password_invalid.yaml: Cyware config needed for the tests.
    ...
    if metadata.get('error') == 'Invalid password provided.':
        pytest.xfail(reason="No password validation in authd.pass - Issue cyware/cyware#16282.")

    # Verify cyware-manager fails at restart.
