...
brief: These tests will check if the 'remote enrollment' option of the 'cyware-authd' daemon
       settings is working properly. The 'cyware-authd' daemon can automatically add
...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/auth.html#remote-enrollment
...
from cyware_testing.tools.sockets import wait_for_tcp_port
from cyware_testing.tools.cyware import DEFAULT_SSL_REMOTE_ENROLLMENT_PORT
...
def test_remote_enrollment(get_configuration, configure_environment, restart_cyware_daemon_function, tear_down):
    '''
    description:
        Checks if the 'cyware-authd' daemon remote enrollment is enabled/disabled according
    ...
    parameters:
        ...
        - restart_authd:
            type: fixture
            brief: Restart the 'cyware-authd' daemon, clear the 'ossec.log' file and start a new file monitor.
        - tear_down:
            type: fixture
    ...
    expected_output:
        - r'Accepting connections on port 1515. No password required.' (When the 'cyware-authd' daemon)
