...
import pytest
from cyware_testing.cluster import FERNET_KEY, CLUSTER_DATA_HEADER_SIZE, cluster_msg_build
from cyware_testing.tools import CYWARE_PATH, CLUSTER_LOGS_PATH
    ...
    def verify_message(self, data: bytes):
        if len(data) > CLUSTER_DATA_HEADER_SIZE:
            ...
            response = cluster_msg_build(command=b'send_sync', counter=2, payload=bytes(self.cluster_output.encode()),
                                         encrypt=False)[0]
            print(f'Received message from cyware-authd: {message}')
...
cluster_socket_path = os.path.join(os.path.join(CYWARE_PATH, 'queue', 'cluster', 'c-internal.sock'))
...
monitored_sockets_params = [('cyware-clusterd', mitm_master, True), ('cyware-authd', None, True)]
