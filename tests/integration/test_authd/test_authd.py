...
brief: These tests will check if the 'cyware-authd' daemon correctly handles the enrollment requests,
       ...
       The 'cyware-authd' daemon can automatically add a Cyware agent to a Cyware manager and provide
...
def test_ossec_auth_messages(get_configuration, set_up_groups, configure_environment, configure_sockets_environment,
                             clean_client_keys_file_module, restart_cyware_daemon, wait_for_authd_startup_module,
                             connect_to_sockets_module):
    '''
    description:
        Checks if when the `cyware-authd` daemon receives different types of enrollment requests,
    ...
    parameters:
        ...
        - clean_client_keys_file_module:
            type: fixture
            brief: Stops Cyware and cleans any previous key in client.keys file at module scope.
        - restart_authd:
            type: fixture
