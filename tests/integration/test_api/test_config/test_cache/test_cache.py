...
brief: These tests will check if the cache feature of the API handled by the 'cyware-apid' daemon
       is working properly. The Cyware API is an open source 'RESTful' API that allows for interaction
...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/api/configuration.html#cache
...
import pytest
import requests
from cyware_testing.fim import create_file, delete_file, REGULAR, CYWARE_PATH
...
rules_directory = os.path.join(CYWARE_PATH, 'ruleset', 'rules')
