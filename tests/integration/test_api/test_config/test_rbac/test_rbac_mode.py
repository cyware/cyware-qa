...
brief: These tests will check if the 'rbac_mode' (Role-Based Access Control) setting of the API
       ...
       'whitelist mode' and 'blacklist mode'. The Cyware API is an open source 'RESTful' API
       that allows for interaction with the Cyware manager from a web browser, command line tool
...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/api/configuration.html#rbac-mode
...
import pytest
import requests
from cyware_testing.fim import CYWARE_PATH
from cyware_testing.tools.configuration import check_apply_test
from cyware_testing.tools.configuration import get_api_conf
...
rbac_sql_path = os.path.join(CYWARE_PATH, 'api', 'configuration', 'security', 'rbac.db')
...
def test_rbac_mode(tags_to_apply, get_configuration, configure_api_environment, restart_api,
    ...
    api_details = get_api_details(user='test_user', password='cyware')
