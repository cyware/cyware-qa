...
brief: These tests will check if the 'max_upload_size' setting of the API is working properly.
       ...
       The Cyware API is an open source 'RESTful' API that allows for interaction with
       the Cyware manager from a web browser, command line tool like 'cURL' or any script
...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/api/configuration.html
...
from cyware_testing.tools import API_LOG_FILE_PATH
from cyware_testing.tools.configuration import check_apply_test, load_cyware_configurations
...
daemons_handler_configuration = {'daemons': ['cyware-apid']}
file_to_monitor = API_LOG_FILE_PATH
test_data_path = join(dirname(realpath(__file__)), 'data')
configurations_path = join(test_data_path, 'cyware_max_upload_size.yaml')
...
def restart_required_api_cyware():
    """Restart API-required services and stop them all at the end of the test."""
    required_api_daemons = ['cyware-modulesd', 'cyware-analysisd', 'cyware-execd', 'cyware-db', 'cyware-remoted']
...
def test_max_upload_size(tags_to_apply, get_configuration, configure_api_environment, restart_required_api_cyware,
    ...
    assertions:
        - Verify that the 'cyware-apid' daemon returns a proper HTTP status code depending on the value
    ...
    input_description: A test case is (test_upload_size) contained in an external YAML file
                       (cyware_max_upload_size.yaml) which includes API configuration
