...
configurations_single = load_cyware_configurations(configurations_path, "test_basic_configuration_allowed_denied_ips",
                                                  params=parameters_single, metadata=metadata_single)
configurations_multiple = load_cyware_configurations(configurations_path,
...
def test_allowed_denied_ips_syslog(get_configuration, configure_environment, restart_remoted, wait_for_remoted_start_log):
    ...
    expected_output:
        ...
        - Cyware remoted didn't start as expected.
    ...
    cyware_log_monitor.start(timeout=remote.REMOTED_GLOBAL_TIMEOUT, callback=log_callback,
                            error_message="Cyware remoted didn't start as expected.")

    if 'allowed-ips2' in cfg:
        pytest.xfail(f"Expected error: https://github.com/cyware/cyware/issues/11643")
