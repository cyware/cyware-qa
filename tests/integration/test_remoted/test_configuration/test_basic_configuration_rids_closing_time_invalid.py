...
brief: The 'cyware-remoted' program is the server side daemon that communicates with the agents.
       Specifically, this test will check if 'cyware-remoted' produces an error message when an
...
from cyware_testing.generic_callbacks import callback_error_invalid_value_for
...
def test_rids_closing_time_invalid(get_configuration, configure_environment, restart_remoted):
    '''
    description: Check if 'cyware-remoted' fails when an invalid 'rids_closing_time' value is set. For this purpose, 
