...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/agents/agent-life-cycle.html?highlight=status#agent-status
...
from cyware_testing import UDP, TCP, TCP_UDP, remote
from cyware_testing.remote import check_push_shared_config, REMOTED_GLOBAL_TIMEOUT
...
configurations_path = os.path.join(test_data_path, 'cyware_agent_version_shared_configuration_startup_message.yaml')
...
def test_agent_remote_configuration(agent_name, get_configuration, configure_environment, remove_shared_files,
    ...
    assertions:
        - Verify that the shared configuration was sent, checking the agent version retrieved by 'cyware_bd'
    ...
    input_description: A configuration template (test_agent_version_shared_configuration_startup_message) is contained
                       in an external YAML file (cyware_agent_version_shared_configuration_startup_message.yaml).
                       That template is combined with different test cases defined in the module. Those include
                       configuration settings for the 'cyware-remoted' daemon and agents info.
    ...
    for protocol in protocols.split(","):
        ...
        cyware_db_agent_version = agent.get_agent_version()
        assert cyware_db_agent_version == fr"Cyware {agent_info[agent_name]['version']}"
