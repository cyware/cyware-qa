...
from cyware_testing import TCP, UDP
...
configurations_path = os.path.join(test_data_path, 'cyware_agents_reconnection.yaml')
disconnect_time = 5
...
def test_agents_switching_protocols(get_configuration, configure_environment, restart_remoted):
    ...
    input_description: A configuration template (test_agents_switching_protocols) is contained in an external YAML file,
                       (cyware_agents_reconnection.yaml). That template is combined with different test cases defined
                       in the module. Those include configuration settings for the 'cyware-remoted' daemon and agents
                       info.
