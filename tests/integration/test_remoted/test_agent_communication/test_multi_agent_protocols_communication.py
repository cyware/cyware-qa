...
import cyware_testing.remote as rd
import cyware_testing.tools.agent_simulator as ag
import cyware_testing.tools.monitoring as mo

from time import sleep
from cyware_testing import TCP, UDP, TCP_UDP
...
configurations_path = os.path.join(test_data_path, 'cyware_multi_agent_protocols_communication.yaml')
...
def test_multi_agents_protocols_communication(get_configuration, configure_environment, restart_cyware):
    '''
    ...
    parameters:
        ...
        - restart_cyware:
            type: fixture
            brief: Stop Cyware, reset ossec.log and start a new monitor. Then start Cyware.
    ...
    input_description: A configuration template (test_multi_agent_protocols_communication) is contained in an external
                       YAML file, (cyware_multi_agent_protocols_communication.yaml). That template is combined with
                       different test cases defined in the module. Those include configuration settings for the
                       'cyware-remoted' daemon and agents info.
