...
brief: Active responses perform various countermeasures to address active
       ...
       correctly to the Cyware agent by `cyware-remoted` daemon.
...
from cyware_testing import UDP, TCP, TCP_UDP
from cyware_testing.tools.configuration import load_cyware_configurations
from cyware_testing.tools.sockets import send_active_response_message
...
configurations_path = os.path.join(test_data_path, 'cyware_test_active_response.yaml')
...
def test_active_response_ar_sending(get_configuration, configure_environment, restart_remoted):
    '''
    description: Check if the 'cyware-remoted' daemon sends active response commands to the Cyware agent.
    ...
    assertions:
        - Verify that the 'cyware-execd' daemon sends the active response to the 'cyware-remoted' daemon.
        - Verify that the 'cyware-remoted' daemon receives the active response from the 'cyware-execd' daemon.
        - Verify that the Cyware agent receives an active response message.
    
    input_description: A configuration template (test_active_response_send_ar) is contained in an external YAML
                       file (cyware_test_active_response.yaml). That template is combined with different
                       test cases defined in the module. Those include configuration settings for
                       the 'cyware-remoted' daemon.
