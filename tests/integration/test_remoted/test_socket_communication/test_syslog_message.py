...
brief: The 'cyware-remoted' program is the server side daemon that communicates with the agents.
       Specifically, this test will check if 'cyware-remoted' can receive syslog messages through
       the socket.
...
from cyware_testing import is_udp
...
configurations_path = os.path.join(test_data_path, 'cyware_syslog.yaml')

syslog_messages = {
    'dummy': "Syslog message sent by cyware-qa to test remoted syslog",
...
def test_syslog_message(message, get_configuration, configure_environment, restart_cyware):
    '''
    description: Check if 'cyware-remoted' can receive syslog messages through the socket.
    ...
    input_description: A configuration template (test_syslog_message) is contained in an external YAML file,
                       (cyware_syslog.yaml). That template is combined with different test cases defined
    ...
    cyware_archives_log_monitor = remote.create_archives_log_monitor()
    ...
    cyware_log_monitor.start(timeout=5, callback=log_callback, update_position=False,
                            error_message="Cyware remoted didn't start as expected.")

    # Check if cyware-remoted receives syslog messages
    remote.check_syslog_event(cyware_archives_log_monitor, syslog_messages[message], port, protocol)
