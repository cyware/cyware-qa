...
brief: The 'cyware-remoted' program is the server side daemon that communicates with the agents.
       Specifically, this test will check if 'cyware-remoted' sends the #pong message.
...
import cyware_testing.remote as rm
from cyware_testing import TCP_UDP
...
configurations_path = os.path.join(test_data_path, 'cyware_socket_communication.yaml')
...
def test_ping_pong_message(get_configuration, configure_environment, restart_remoted):
    '''
    description: Check if 'cyware-remoted' sends the #pong message
    ...
    input_description: A configuration template (test_ping_pong_message) is contained in an external YAML file,
                       (cyware_socket_communication.yaml). That template is combined with different test cases defined
    ...
    cyware_log_monitor.start(timeout=5, callback=log_callback, error_message="Cyware remoted didn't start as expected.")
