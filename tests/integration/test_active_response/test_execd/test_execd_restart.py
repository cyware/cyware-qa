...
CLIENT_KEYS_PATH = os.path.join(CYWARE_PATH, CONF_FOLDER, 'client.keys')
SERVER_KEY_PATH = os.path.join(CYWARE_PATH, CONF_FOLDER, 'manager.key')
SERVER_CERT_PATH = os.path.join(CYWARE_PATH, CONF_FOLDER, 'manager.cert')
CRYPTO = "aes"
...
def wait_message_line(line):
    ...
    if CURRENT_PLATFORM == 'Windows' and "active-response/bin/restart-cyware.exe: {\"version\"" in line:
        return True
    elif "active-response/bin/restart-cyware: {\"version\"" in line:
        return True
    return None
...
def wait_shutdown_message_line(line):
    """Callback function to wait for Cyware shutdown message."""
...
def test_execd_restart(set_debug_mode, get_configuration, test_version,
    ...
    '''
    description: Check if 'restart-cyware' command of 'active response' is executed correctly.
                 For this purpose, a simulated agent is used, to which the active response is sent.
                 This response includes the order to restart the Cyware agent,
