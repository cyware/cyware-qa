...
from cyware_testing.processes import check_if_daemons_are_running, run_local_command_printing_output
...
from cyware_testing.tools import CUSTOM_RULES_PATH, LOCAL_RULES_PATH, ACTIVE_RESPONSE_BINARY_PATH
...
cyware_ar_script = os.path.join(ACTIVE_RESPONSE_BINARY_PATH, 'custom-ar.sh')
output_custom_ar_script = '/tmp/file-ar.txt'
source_path = [custom_rule, local_rules, custom_ar_script]
destination_path = [f"{CUSTOM_RULES_PATH}/0270-web_appsec_rules_edit.xml", LOCAL_RULES_PATH, cyware_ar_script]
...
def test_overwritten_rules_ar(configuration, metadata, source_path, destination_path, new_file_path,
                              create_file, set_cyware_configuration, copy_file, truncate_monitored_files,
                              restart_cyware_function, clean_AR_file):
    '''
    ...
    test_phases:
        - setup:
            - Copy custom rule and active response files to Cyware paths.
            - Create a new file which will be monitored with logcollector.
            - Set cyware configuration. Add active_response, command and localfile blocks.
        ...
        - teardown:
            ...
            - Restart initial cyware configuration.
    ...
    parameters:
        ...
        - create_file:
            type: fixture
            brief: Create a file to be monitored by Cyware.
        - set_cyware_configuration:
            type: fixture
            brief:  Set Cyware custom configuration.
        - copy_file:
            type: fixture
            brief:  Copy file from source to destination.
        - truncate_monitored_files:
            type: fixture
            brief:  Truncate cyware log files.
        - restart_cyware_function:
            type: fixture
            brief: restart Cyware.
        - clean_AR_file:
            type: fixture
    ...
    '''
    # Set all permissions for cyware to be able to use the custom active response
    os.chmod(cyware_ar_script, 0o777)
    ...
    # Check that cyware-analysisd is running and has not crashed
    assert check_if_daemons_are_running(['cyware-analysisd'])[0], 'cyware-analysisd daemon is not running. ' \
