...
from cyware_testing.fim import CHECK_OWNER, LOG_FILE_PATH, registry_value_cud, registry_key_cud, \
...
configurations_path = os.path.join(test_data_path, 'cyware.khulnasoft.complex_entries.yaml')
...
def test_ambiguous_complex_checks(key, subkey, key_checkers,
    ...
    input_description: A test case (complex_checks) is contained in an external YAML file
                       (cyware.khulnasoft.complex_entries.yaml) which includes configuration settings
                       for the 'cyware-syscheckd' daemon. That is combined with the
    ...
    # Test registry keys.
    registry_key_cud(key, subkey, cyware_log_monitor, min_timeout=global_parameters.default_timeout,
                     options=key_checkers, time_travel=True)

    # Test registry values.
    registry_value_cud(key, subkey, cyware_log_monitor, min_timeout=global_parameters.default_timeout,
                       options=key_checkers, time_travel=True)
...
def test_ambiguous_report_changes(key, subkey, value_list, report,
                                  get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon generates or not the 'content_changes' field for each event
...
def test_ambiguous_report_tags(key, subkey, tag,
                               get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon generates or not the 'tags' field for each event
