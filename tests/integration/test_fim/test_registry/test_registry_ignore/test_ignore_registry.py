...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/syscheck.html#registry-ignore
...
import pytest
from cyware_testing import T_20, fim
...
configurations_path = os.path.join(test_data_path, 'cyware_registry_ignore_conf.yaml')
...
def test_ignore_registry_key(root_key, registry, arch, subkey, triggers_event, tags_to_apply,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon ignores the events from keys that are in a monitored subkey
    ...
    input_description: A test case (ignore_registry_key) is contained in an external YAML file
                       (cyware_registry_ignore_conf.yaml) which includes configuration settings
...
def test_ignore_registry_value(root_key, registry, arch, value, triggers_event, tags_to_apply,
                               get_configuration, configure_environment, restart_syscheckd, wait_for_fim_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon ignores the events from values that are in a monitored subkey
    ...
    # Let syscheck perform a new scan
    if triggers_event:
        event = cyware_log_monitor.start(timeout=T_20, callback=fim.callback_value_event,
