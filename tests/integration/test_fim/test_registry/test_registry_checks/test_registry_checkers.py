...
from cyware_testing.fim import CHECK_GROUP, CHECK_MTIME, CHECK_OWNER, CHECK_PERM, \
...
configurations_path = os.path.join(test_data_path, 'cyware_check_all.yaml')
...
def test_checkers(key, subkey, arch, key_attrs, value_attrs, tags_to_apply, triggers_modification,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon adds in the generated events the 'check_' specified in
    ...
    input_description: Different test cases are contained in an external YAML file (cyware_check_all.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon. Those are
    ...
    # Test registry keys.
    registry_key_cud(key, subkey, cyware_log_monitor, arch=arch, min_timeout=global_parameters.default_timeout,
