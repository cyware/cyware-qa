...
from cyware_testing.fim import LOG_FILE_PATH, calculate_registry_diff_paths, registry_value_cud, KEY_WOW64_32KEY, \
...
configurations_path = os.path.join(test_data_path, 'cyware_registry_report_changes.yaml')
...
def test_report_changes(key, subkey, arch, value_name, tags_to_apply,
                        ...
                        wait_for_fim_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon detects events and creates 'diff' files from monitored
    ...
    input_description: A test case (test_report_changes) is contained in external YAML file
                       (cyware_registry_report_changes.yaml) which includes configuration
