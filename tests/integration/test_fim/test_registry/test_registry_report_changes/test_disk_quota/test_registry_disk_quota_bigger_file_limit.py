...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the 'queue/diff/local' folder where Cyware stores the compressed files used to perform
...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/syscheck.html#disk-quota
...
import pytest
from cyware_testing import LOG_FILE_PATH, global_parameters
from cyware_testing.modules.fim import (WINDOWS_HKEY_LOCAL_MACHINE, MONITORED_KEY, MONITORED_KEY_2, KEY_WOW64_32KEY,
                                       KEY_WOW64_64KEY, SIZE_LIMIT_CONFIGURED_VALUE)
from cyware_testing.modules.fim.event_monitor import ERR_MSG_CONTENT_CHANGES_EMPTY, ERR_MSG_CONTENT_CHANGES_NOT_EMPTY
from cyware_testing.modules.fim.utils import (generate_params, calculate_registry_diff_paths, registry_value_create,
...
configurations_path = os.path.join(test_data_path, "cyware_registry_report_changes_limits_quota.yaml")
...
def test_disk_quota_values(key, subkey, arch, value_name, size, get_configuration, configure_environment,
    ...
    """
    description: Check if the 'cyware-syscheckd' daemon sets the 'disk_quota' limit to be equal to the
    ...
    parameters:
        ...
        - restart_syscheckd:
            type: fixture
            brief: Clear the Cyware logs file and start a new monitor.
        - wait_for_fim_start:
            type: fixture
    ...
    input_description: A test case (test_limits) is contained in external YAML file
                       (cyware_registry_report_changes_limits_quota.yaml) which includes configuration
                       settings for the 'cyware-syscheckd' daemon. That is combined with
    ...
    registry_value_create(key, subkey, cyware_log_monitor, arch=arch, value_list=values, wait_for_scan=True,
    ...
    registry_value_update(key, subkey, cyware_log_monitor, arch=arch, value_list=values, wait_for_scan=True,
                          scan_delay=scan_delay, min_timeout=global_parameters.default_timeout, triggers_event=True,
                          validators_after_update=[test_callback])
    # Delete the value created to clean up enviroment
    registry_value_delete(key, subkey, cyware_log_monitor, arch=arch, value_list=values, wait_for_scan=True,
                          scan_delay=scan_delay, min_timeout=global_parameters.default_timeout, triggers_event=True)
