...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these files
       ...
       monitored to generate 'diff' information or the 'queue/diff/local' folder where Cyware stores the
...
from cyware_testing.modules.fim import (WINDOWS_HKEY_LOCAL_MACHINE, MONITORED_KEY, MONITORED_KEY_2, KEY_WOW64_32KEY,
                                       KEY_WOW64_64KEY)
from cyware_testing.modules.fim.event_monitor import ERR_MSG_CONTENT_CHANGES_EMPTY
from cyware_testing.modules.fim.utils import (generate_params, calculate_registry_diff_paths, create_values_content,
...
def test_all_limits_disabled(key, subkey, arch, value_name, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon generates all FIM events when the 'file_size' and
    ...
    input_description: A test case (test_limits) is contained in external YAML file
                       (cyware_registry_report_changes_limits_quota.yaml) which includes
                       configuration settings for the 'cyware-syscheckd' daemon. That is
