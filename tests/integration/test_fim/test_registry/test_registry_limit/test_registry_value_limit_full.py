...
from cyware_testing.modules import WINDOWS, TIER1
from cyware_testing.modules.fim import (registry_parser, KEY_WOW64_64KEY, REG_SZ, KEY_ALL_ACCESS, RegOpenKeyEx,
...
from cyware_testing.modules.fim.event_monitor import (CB_REGISTRY_LIMIT_CAPACITY, CB_COUNT_REGISTRY_VALUE_ENTRIES,
...
from cyware_testing.modules.fim.utils import generate_params, modify_registry_value, create_registry
...
def test_registry_value_limit_full(configure_local_internal_options_module, get_configuration, configure_environment,
                                   restart_syscheckd):
    '''
    description: Check if the 'cyware-syscheckd' daemon generates proper events while the FIM database is in
    ...
    input_description: A test case (fim_registry_limit) is contained in external YAML file (cyware_conf.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon. That is combined
    ...
    tags:
        - scheduled
    '''
    database_state = cyware_log_monitor.start(timeout=global_parameters.default_timeout,
    ...
    cyware_log_monitor.start(timeout=monitor_timeout,
    ...
    value_entries = cyware_log_monitor.start(timeout=monitor_timeout,
