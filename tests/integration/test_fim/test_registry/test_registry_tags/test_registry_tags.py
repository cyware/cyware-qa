...
from cyware_testing.fim import LOG_FILE_PATH, registry_value_cud, generate_params, KEY_WOW64_64KEY, KEY_WOW64_32KEY
...
configurations_path = os.path.join(test_data_path, 'cyware_registry_tag_conf.yaml')
...
def test_tags(key, subkey, arch,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon generates the tags required for each event depending
    ...
    input_description: A test case (ossec_conf) is contained in external YAML file (cyware_registry_tag_conf.yaml)
    ...
    registry_value_cud(key, subkey, cyware_log_monitor, arch=arch,
