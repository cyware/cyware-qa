...
from cyware_testing.fim import LOG_FILE_PATH, generate_params, create_registry, modify_registry_value, delete_registry, \
...
configurations_path = os.path.join(test_data_path, 'cyware_conf_registry_both.yaml')
...
def test_delete_registry(key, subkey, arch, value_list,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon detects 'deleted' events from the values contained
    ...
    input_description: A test case (ossec_conf_2) is contained in an external YAML file
                       (cyware_conf_registry_both.yaml) which includes configuration settings for
    ...
    events = cyware_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_value_event,
    ...
    event_list = cyware_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_value_event,
