...
import sys
import pytest
from cyware_testing import LOG_FILE_PATH, DATA, CYWARE_SERVICES_START
...
from cyware_testing.modules.fim.utils import (find_value_in_event_list, get_sync_msgs, generate_params, create_registry,
                                             modify_registry_value)
from cyware_testing.modules.fim import (FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS, SCHEDULED_MODE, WINDOWS_REGISTRY,
...
from cyware_testing.modules.fim.event_monitor import detect_initial_scan
...
def test_registry_sync_after_restart(key_name, value_name, configure_local_internal_options_module,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon synchronizes the registry DB when a modification
    ...
    input_description: A test case (registry_sync_responses) is contained in external YAML file
                       (cyware_conf_registry_responses_win32.yaml) which includes configuration
                       settings for the 'cyware-syscheckd' daemon. That is combined with the
    ...
    detect_initial_scan(cyware_log_monitor)
