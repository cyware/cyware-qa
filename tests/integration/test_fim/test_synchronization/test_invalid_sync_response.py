...
from cyware_testing.fim import LOG_FILE_PATH, callback_configuration_warning
...
configurations_path = os.path.join(test_data_path, 'cyware_invalid_conf.yaml')
...
def test_invalid_sync_response(get_configuration, configure_environment, restart_syscheckd):
    '''
    description: Check if the 'cyware-syscheckd' daemon detects invalid synchronization intervals
    ...
    input_description: A test case (sync_invalid) is contained in external YAML file (cyware_invalid_conf.yaml)
    ...
    cyware_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_configuration_warning,
