...
import pytest
from cyware_testing import T_10, T_20
from cyware_testing.fim import LOG_FILE_PATH, delete_file, generate_params, create_file, REGULAR
...
from cyware_testing.modules.fim.event_monitor import (callback_detect_event, ERR_MSG_DATABASE_FULL_ALERT_EVENT,
...
configurations_path = os.path.join(test_data_path, 'cyware_conf_delete_full.yaml')
...
def test_file_limit_delete_full(folder, file_name, configure_local_internal_options_module, get_configuration,
    ...
    input_description: A test case (tags_delete_full) is contained in external YAML file (cyware_conf_delete_full.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon and, it is combined with
    ...
    database_state = cyware_log_monitor.start(timeout=T_20,
    ...
    with pytest.raises(TimeoutError):
        event = cyware_log_monitor.start(timeout=T_10,
    ...
    event = cyware_log_monitor.start(timeout=T_20,
