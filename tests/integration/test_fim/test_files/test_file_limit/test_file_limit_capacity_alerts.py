...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/syscheck.html#file-limit
...
from cyware_testing import LOG_FILE_PATH, REGULAR, global_parameters
from cyware_testing.tools import PREFIX
from cyware_testing.tools.configuration import load_cyware_configurations
from cyware_testing.tools.file import create_file, delete_file
from cyware_testing.tools.monitoring import FileMonitor, generate_monitoring_callback
from cyware_testing.modules import TIER1
from cyware_testing.modules.fim import SCHEDULED_MODE, FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS
from cyware_testing.modules.fim.event_monitor import (callback_entries_path_count, CB_FILE_LIMIT_CAPACITY,
...
from cyware_testing.modules.fim.utils import generate_params, wait_for_scheduled_scan
...
def test_file_limit_capacity_alert(percentage, configure_local_internal_options_module, get_configuration,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon generates events for different capacity thresholds limits when
    ...
    input_description: A test case (file_limit_conf) is contained in external YAML file (cyware_conf.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon and, it is
    ...
    wait_for_scheduled_scan(True, interval=scan_delay, monitor=cyware_log_monitor)

    # Look for file_limit percentage alert configure value and check it matches with the expected percentage
    if percentage >= 80:
        file_limit_capacity = cyware_log_monitor.start(timeout=global_parameters.default_timeout,
    ...
    entries, path_count = cyware_log_monitor.start(timeout=global_parameters.default_timeout,
