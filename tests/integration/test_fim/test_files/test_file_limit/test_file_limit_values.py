...
from cyware_testing.modules.fim.event_monitor import (ERR_MSG_FILE_LIMIT_VALUES, CB_FILE_LIMIT_VALUE,
...
def test_file_limit_values(configure_local_internal_options_module, get_configuration, configure_environment,
                           restart_syscheckd):
    '''
    description: Check if the 'cyware-syscheckd' daemon detects that the value of the 'entries' tag, which corresponds
    ...
    tags:
        - scheduled
    '''
    # Get the file_limit value configured from the cyware logs
    file_limit_value = cyware_log_monitor.start(timeout=global_parameters.default_timeout,
    ...
    entries, path_count = cyware_log_monitor.start(timeout=monitor_timeout, callback=callback_entries_path_count,
