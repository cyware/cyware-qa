...
from cyware_testing.fim import callback_audit_event_too_long
...
conf_name = "cyware_recursion_windows.yaml" if sys.platform == "win32" else "cyware_recursion.yaml"
    ...
    try:
        ...
        for n in range(recursion_level):
            ...
            if ((recursion_level < edge_limit * 2) or
                ...
                regular_file_cud(path, cyware_log_monitor, time_travel=is_scheduled, min_timeout=timeout)
...
def test_recursion_level(dirname, subdirname, recursion_level, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon detects events in a monitored directories hierarchy with
    ...
    input_description: A test case (test_recursion_level) is contained in external YAML files
                       (cyware_recursion.yaml or cyware_recursion_windows.yaml) which includes
                       configuration settings for the 'cyware-syscheckd' daemon and the directories
