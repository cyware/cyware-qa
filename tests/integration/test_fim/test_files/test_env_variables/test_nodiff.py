...
from cyware_testing.modules.fim.utils import regular_file_cud, generate_params
from cyware_testing.tools.configuration import load_cyware_configurations, PREFIX
...
configurations_path = os.path.join(test_data_path, 'cyware_conf_nodiff.yaml')
mark_skip_agentWindows = pytest.mark.skipif(sys.platform == 'win32', reason="It will be blocked by cyware/cyware-qa#2174")
...
def test_tag_nodiff(directory, filename, hidden_content, get_configuration, put_env_variables, configure_environment,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon truncates the content in the 'diff' files when testing files
    ...
    input_description: A test case (ossec_conf) is contained in external YAML file (cyware_conf_nodiff.yaml) which
    ...
    regular_file_cud(directory, cyware_log_monitor, file_list=files,
