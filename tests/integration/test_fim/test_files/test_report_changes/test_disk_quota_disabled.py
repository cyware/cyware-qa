...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when
       ...
       the size of the 'queue/diff/local' folder where Cyware stores the compressed files used
...
from cyware_testing.modules.fim.event_monitor import callback_disk_quota_limit_reached
from cyware_testing.modules.fim.utils import generate_params, create_file
...
def test_disk_quota_disabled(filename, folder, size, get_configuration, configure_environment,
    ...
    input_description: A test case (ossec_conf_diff) is contained in external YAML file (cyware_conf.yaml)
    ...
    with pytest.raises(TimeoutError):
        cyware_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_disk_quota_limit_reached)
