...
from cyware_testing import global_parameters, LOG_FILE_PATH, REGULAR, T_20
from cyware_testing.modules.fim import TEST_DIR_1
from cyware_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS as local_internal_options
from cyware_testing.modules.fim.event_monitor import (callback_detect_event, get_fim_event,
                                                     callback_detect_file_more_changes)
from cyware_testing.modules.fim.utils import create_file
...
def test_large_changes(configuration, metadata, set_cyware_configuration, configure_local_internal_options_function,
                       create_monitored_folders_module, restart_syscheck_function, wait_syscheck_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon detects the character limit in the file changes is reached
    ...
    cyware_log_monitor.start(timeout=T_20, callback=callback_detect_event,
    ...
    if metadata['has_more_changes']:
        ...
        event = cyware_log_monitor.start(timeout=T_20, callback=callback_detect_event,
                                        error_message="Did not receive the expected FIM event").result()
