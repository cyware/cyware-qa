...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/syscheck.html#diff
...
from cyware_testing.tools.configuration import get_cyware_conf, set_section_cyware_conf, load_cyware_configurations
from cyware_testing.tools.file import create_file
...
from cyware_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS
...
def change_conf(report_value):
    ...
    return load_cyware_configurations(configurations_path, __name__, params=conf_params, metadata=conf_metadata)
...
def detect_fim_scan(file_monitor, fim_mode):
    """
    Detect initial scan when restarting Cyware.
...
def disable_report_changes(fim_mode):
    ...
    new_ossec_conf = set_section_cyware_conf(new_conf[0].get('sections'))

    restart_cyware_with_new_conf(new_ossec_conf)
    # Wait for FIM scan to finish
    detect_fim_scan(cyware_log_monitor, fim_mode)
...
def test_report_when_deleted_directories(path, get_configuration, configure_environment,
                                         ...
                                         wait_for_fim_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon deletes the 'diff' folder created in the 'queue/diff/local'
    ...
    input_description: Different test cases are contained in external YAML file (cyware_conf.yaml) which
                       includes configuration settings for the 'cyware-syscheckd' daemon and, these
...
def test_report_changes_after_restart(get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon deletes the 'diff' folder created in the 'queue/diff/file'
                 ...
                 directory will be deleted after the 'cyware-syscheckd' daemon restart but will be created again if
                 the 'report_changes' option is still active. To avoid this, the test will disable the 'report_changes'
                 option (backing the main configuration) before restarting the 'cyware-syscheckd' daemon to ensure that
    ...
    try:
        ...
        restart_cyware_with_new_conf(backup_conf)
        detect_fim_scan(cyware_log_monitor, fim_mode)
