...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/syscheck.html#nodiff
...
def test_reports_file_and_nodiff(configuration, metadata, set_cyware_configuration,
                                 configure_local_internal_options_function, restart_syscheck_function,
                                 create_monitored_folders_module, wait_syscheck_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon reports the file changes (or truncates if required)
    ...
    input_description: A test case (ossec_conf_report) is contained in external YAML file (cyware_conf.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon and, these
    ...
    cyware_log_monitor = FileMonitor(LOG_FILE_PATH)
    regular_file_cud(folder, cyware_log_monitor, file_list=file_list, min_timeout=T_20,
