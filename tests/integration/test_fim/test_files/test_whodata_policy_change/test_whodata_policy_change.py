...
import pytest
from cyware_testing.tools import PREFIX, configuration
...
from cyware_testing import T_5, T_20, T_30, LOG_FILE_PATH
from cyware_testing.modules import fim
...
from cyware_testing.modules.fim.utils import regular_file_cud
...
def test_whodata_policy_change(configuration, metadata, set_cyware_configuration, create_monitored_folders_module,
    ...
    '''
    description: Check if the 'cyware-syscheckd' is monitoring a in whodata mode in Windows, and the Audit Policies are
    ...
    evm.detect_windows_sacl_configured(cyware_log_monitor)
    # Check Whodata engine has started
    evm.detect_whodata_start(cyware_log_monitor)
    ...
    evm.detect_windows_whodata_mode_change(cyware_log_monitor)

    # Create/Update/Delete file and check events
    cyware_log_monitor = FileMonitor(LOG_FILE_PATH)
    regular_file_cud(folder, cyware_log_monitor, file_list=file_list, event_mode=fim.REALTIME_MODE,
