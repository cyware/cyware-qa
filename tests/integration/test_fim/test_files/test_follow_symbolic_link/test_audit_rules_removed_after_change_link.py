...
def test_audit_rules_removed_after_change_link(replaced_target, new_target, file_name, tags_to_apply,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon removes the 'audit' rules when the target of
    ...
    input_description: A test case (check_audit_removed_rules) is contained in external YAML file (cyware_conf.yaml)
    ...
    ev = cyware_log_monitor.start(timeout=global_parameters.default_timeout, callback=fim.callback_detect_event,
    ...
    fim.wait_for_audit(True, cyware_log_monitor)
