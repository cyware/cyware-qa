...
def test_symbolic_delete_symlink(tags_to_apply, main_folder, aux_folder, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyware-syscheckd' stops detecting events when deleting the monitored symlink.
    ...
    input_description: Two test cases (monitored_file and monitored_dir) are contained in external YAML file
                       (cyware_conf.yaml) which includes configuration settings for the 'cyware-syscheckd' daemon
    ...
    if tags_to_apply == {'monitored_dir'}:
        ...
        cyware_log_monitor.start(timeout=10, callback=fim.callback_detect_event,
    ...
    with pytest.raises(TimeoutError):
        event = cyware_log_monitor.start(timeout=10, callback=fim.callback_detect_event)
    ...
    fim.wait_for_audit(get_configuration['metadata']['fim_mode'] == "whodata", cyware_log_monitor)

    fim.modify_file_content(main_folder, file1, new_content='Sample modification 2')
    fim.check_time_travel(scheduled, monitor=cyware_log_monitor)
    modify = cyware_log_monitor.start(timeout=10, callback=fim.callback_detect_event).result()
