...
def test_symbolic_change_target(tags_to_apply, main_folder, aux_folder, get_configuration, configure_environment,
    ...
    '''
    description: Check if the 'cyware-syscheckd' updates the symlink target properly. For this purpose,
    ...
    def modify_and_check_events(f1, f2, text):
        ...
        modify = cyware_log_monitor.start(timeout=3, callback=fim.callback_detect_event,
                                         error_message='Did not receive expected "Sending FIM event: ..." event'
                                         ).result()
        assert 'modified' in modify['data']['type'] and f1 in modify['data']['path'], \
            f"'modified' event not matching for {file1}"
        with pytest.raises(TimeoutError):
            event = cyware_log_monitor.start(timeout=3, callback=fim.callback_detect_event)
    ...
    if main_folder == testdir_target:
        ...
        fim.check_time_travel(scheduled, monitor=cyware_log_monitor)
        add = cyware_log_monitor.start(timeout=10, callback=fim.callback_detect_event,
                                      error_message='Did not receive expected "Sending FIM event: ..." event'
                                      ).result()
