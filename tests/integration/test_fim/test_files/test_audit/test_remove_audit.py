...
brief: These tests will check if the 'cyware-syscheckd' and 'auditd' daemons work together properly.
...
from cyware_testing.modules.fim.event_monitor import callback_audit_cannot_start
...
def test_move_folders_to_realtime(configuration, metadata, set_cyware_configuration, create_monitored_folders,
    ...
    test_phases:
        - setup:
            ...
            - Restart cyware-syscheck daemon to apply configuration changes.
        - test:
    ...
    input_description: A test case is contained in external YAML file (configuration_remove_audit.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon
    ...
    cyware_log_monitor.start(timeout=20, callback=callback_audit_cannot_start,
