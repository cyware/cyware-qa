...
from cyware_testing.fim import (LOG_FILE_PATH, REGULAR, callback_detect_event, create_file)
...
def check_event(dirsrc, dirdst, filename, mod_del_event, mod_add_event):
    ...
    mod_add_event : str
        Mode of added event.
    """
    event = cyware_log_monitor.start(timeout=global_parameters.default_timeout, callback=callback_detect_event).result()
...
def test_moving_file_to_whodata(dirsrc, dirdst, filename, mod_del_event, mod_add_event, get_configuration,
    ...
    '''
    description: Check if the 'cyware-syscheckd' daemon detects events when moving files from a directory
    ...
    input_description: A test case (monitoring_realtime) is contained in external YAML file (cyware_conf.yaml)
