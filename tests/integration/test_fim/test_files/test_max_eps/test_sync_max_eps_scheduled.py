...
from cyware_testing.tools import PREFIX, LOG_FILE_PATH
...
from cyware_testing.modules import DATA, TIER1, AGENT, WINDOWS, LINUX
from cyware_testing.modules.fim import TEST_DIR_1, TEST_DIRECTORIES, YAML_CONF_MAX_EPS_SYNC, SCHEDULED_MODE
from cyware_testing.modules.fim import FIM_DEFAULT_LOCAL_INTERNAL_OPTIONS as local_internal_options
from cyware_testing.modules.fim.event_monitor import (callback_detect_integrity_check_global,
                                                     callback_integrity_sync_message, ERR_MSG_INTEGRITY_CONTROL_MSG,
                                                     ERR_MSG_MULTIPLE_FILES_CREATION)
from cyware_testing.tools.file import delete_path_recursively, write_file
...
def test_max_eps_sync_valid_within_range(configure_local_internal_options_module, get_configuration,
                                         create_multiple_files, configure_environment, restart_cyware):
    '''
    description: Check if the 'cyware-syscheckd' daemon applies the limit set in the 'max_eps' tag when
    ...
    input_description: A test case (max_eps_synchronization) is contained in external YAML file
                       (cyware_sync_conf_max_eps.yaml) which includes configuration settings for
    ...
    try:
        ...
        cyware_log_monitor.start(timeout=TIMEOUT_CHECK_INTEGRATY_START,
        ...
        result = cyware_log_monitor.start(timeout=TIMEOUT_CHECK_EACH_INTEGRITY_MSG, accum_results=total_file_created,
