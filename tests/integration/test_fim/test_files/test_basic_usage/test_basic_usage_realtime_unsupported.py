...
from cyware_testing.fim import detect_initial_scan, callback_ignore_realtime_flag
...
configurations_path = os.path.join(test_data_path, 'cyware_conf_check_realtime.yaml')
...
daemons_handler_configuration = {'daemons': ['cyware-syscheckd']}
...
def test_realtime_unsupported(get_configuration, configure_environment, file_monitoring,
    ...
    parameters:
        ...
        - daemons_handler_module:
            type: fixture
            brief: Handle the Cyware daemons.

    assertions:
        - Verify that FIM changes the monitoring mode from 'realtime' to 'scheduled' when it is not supported.

    input_description: A test case (ossec_conf) is contained in external YAML file (cyware_conf_check_realtime.yaml)
                       which includes configuration settings for the 'cyware-syscheckd' daemon and, it is combined
