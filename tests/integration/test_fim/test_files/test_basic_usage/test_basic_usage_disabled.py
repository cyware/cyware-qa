...
brief: File Integrity Monitoring (FIM) system watches selected files and triggering alerts when these
       files are modified. Specifically, these tests will verify that when the 'cyware-syscheckd' daemon
...
import pytest
from cyware_testing import T_10, LOG_FILE_PATH
...
from cyware_testing.modules.fim.event_monitor import callback_detect_end_scan
from cyware_testing.modules.fim.utils import generate_params, regular_file_cud
...
configurations_path = os.path.join(test_data_path, 'cyware_conf_disabled.yaml')
...
def test_disabled(get_configuration, configure_environment, restart_syscheckd):
    '''
    description: Check if the 'cyware-syscheckd' daemon generates FIM events when it is disabled
    ...
    assertions:
        - Verify that when the 'cyware-syscheckd' daemon is disabled, no FIM events are generated.

    input_description: A test case is contained in external YAML file (cyware_conf_disabled.yaml) which
                       includes configuration settings for the 'cyware-syscheckd' daemon and, it is combined
    ...
    with pytest.raises(TimeoutError):
        event = cyware_log_monitor.start(timeout=T_10, callback=callback_detect_end_scan)
        raise AttributeError(f'Unexpected event {event}')

    # Use 'regular_file_cud' and don't expect any event
    regular_file_cud(testdir, cyware_log_monitor, min_timeout=T_10, triggers_event=False)
