...
import pytest
from cyware_testing import T_10
...
configurations_path = os.path.join(test_data_path,
                                   'cyware_conf.yaml' if sys.platform != 'win32' else 'cyware_conf_win32.yaml')
...
def test_create_after_delete(tags_to_apply, get_configuration, configure_environment, restart_syscheckd,
    ...
    input_description: A test case (ossec_conf) is contained in external YAML file
                       (cyware_conf.yaml or cyware_conf_win32.yaml) which includes configuration
                       settings for the 'cyware-syscheckd' daemon and, it is combined with
                       the testing directories to be monitored defined in this module.

    expected_output:
        - r'.*Sending FIM event: (.+)$' (Initial scan when restarting Cyware)
    ...
    regular_file_cud(directory_str, cyware_log_monitor, file_list=['file1', 'file2', 'file3'],
    ...
    regular_file_cud(directory_str, cyware_log_monitor, file_list=['file4', 'file5', 'file6'],
                     min_timeout=T_10, triggers_event=True)
