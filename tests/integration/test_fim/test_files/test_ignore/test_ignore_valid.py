...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/ossec-conf/syscheck.html#ignore
...
from cyware_testing.fim import LOG_FILE_PATH, callback_detect_event, callback_ignore, create_file, REGULAR, \
...
configurations_path = os.path.join(test_data_path,
                                   'cyware_conf_win32.yaml' if sys.platform == 'win32' else 'cyware_conf.yaml')
...
@pytest.mark.skip(reason="It will be blocked by cyware/cyware#9298, when it was solve we can enable again this test")
def test_ignore_subdirectory(folder, filename, content, triggers_event,
                             tags_to_apply, get_configuration,
                             configure_environment, restart_syscheckd,
                             wait_for_fim_start):
    '''
    description: Check if the 'cyware-syscheckd' daemon ignores the files that are in a monitored subdirectory
    ...
    input_description: Different test cases are contained in external YAML files
                       (cyware_conf.yaml or cyware_conf_win32.yaml) which includes configuration settings for
                       the 'cyware-syscheckd' daemon and, these are combined with the testing directories
    ...
    check_time_travel(scheduled, monitor=cyware_log_monitor)

    if triggers_event:
        event = cyware_log_monitor.start(timeout=global_parameters.default_timeout * 2,
        ...
        while True:
            ignored_file = cyware_log_monitor.start(timeout=global_parameters.default_timeout * 2,
