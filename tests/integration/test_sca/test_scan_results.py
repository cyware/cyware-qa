...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/capabilities/sec-config-assessment/index.html
...
from cyware_testing.modules.sca import event_monitor as evm
from cyware_testing.modules.sca import SCA_DEFAULT_LOCAL_INTERNAL_OPTIONS as local_internal_options
...
def test_sca_scan_results(configuration, metadata, prepare_cis_policies_file, truncate_monitored_files,
                          set_cyware_configuration, configure_local_internal_options_function, restart_cyware_function,
    ...
    evm.check_sca_scan_started(cyware_log_monitor)

    # Check the regex engine used by SCA
    engine = evm.get_scan_regex_engine(cyware_log_monitor)
    ...
    evm.get_sca_scan_rule_id_results(file_monitor=cyware_log_monitor, results_num=int(metadata['results']))

    # Get scan summary event and check it matches with the policy file used
    summary = evm.get_sca_scan_summary(file_monitor=cyware_log_monitor)
