...
brief: Cyware-db is the daemon in charge of the databases with all the Cyware persistent information, exposing a socket
       ...
       Cyware-db confirms that is able to save, update and erase the necessary information into the corresponding
...
from cyware_testing.tools.monitoring import make_callback, CYWARE_DB_PREFIX
from cyware_testing.tools.services import control_service, delete_dbs
...
CYWARE_DB_CHECKSUM_CALCULUS_TIMEOUT = 20
...
def prepare_range_checksum_data():
    ...
    execute_cyware_db_query(command+json.dumps(payload))
...
def execute_cyware_db_query(command, single_response=True):
    """Function to send a command to the cyware-db socket.
...
def validate_cyware_db_response(expected_output, response):
    """ Method to validate the Cyware-DB response.
...
def insert_agent(agent_id, agent_name='TestName'):
    ...
    data = execute_cyware_db_query(command).split()
...
def remove_agent(agent_id):
    ...
    Args:
        agent_id(int): Unique identifier of an agent
    """
    data = execute_cyware_db_query(f"global delete-agent {agent_id}").split()
...
def pre_insert_packages():
    ...
    for pkg_n in range(PACKAGES_NUMBER):
        ...
        'Cyware cyware@cyware.khulnasoft.com',NULL,'{random.randint(1,10)}.0.0','all',NULL,NULL,'Test package {pkg_n}',\
...
def test_cyware_db_messages_agent(restart_cyware, clean_registered_agents, configure_sockets_environment,
                                 connect_to_sockets_module, insert_agents_test, test_case):
    '''
    description: Check that every input agent message in cyware-db socket generates the proper output to cyware-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of cywaredb.
...
def test_cyware_db_messages_global(connect_to_sockets_module, restart_cyware, test_case):
    '''
    description: Check that every global input message in cyware-db socket generates the proper output to cyware-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_cyware_db_chunks(restart_cyware, configure_sockets_environment, clean_registered_agents,
    ...
    description: Check that commands by chunks work properly when the agents' amount exceeds the response maximum size.
                 To do this, it sends a command to the cyware-db socket and checks the response from the socket.
    ...
    def send_chunk_command(command):
        response = execute_cyware_db_query(command)
        status = response.split()[0]
...
def test_cyware_db_range_checksum(restart_cyware, configure_sockets_environment, connect_to_sockets_module,
    ...
    execute_cyware_db_query(command)
    log_monitor.start(timeout=CYWARE_DB_CHECKSUM_CALCULUS_TIMEOUT,
                      callback=make_callback('range checksum: Time: ', prefix=CYWARE_DB_PREFIX,
                                             escape=True),
    ...
    execute_cyware_db_query(command)
    log_monitor.start(timeout=CYWARE_DB_CHECKSUM_CALCULUS_TIMEOUT,
                      callback=make_callback('range checksum avoided', prefix=CYWARE_DB_PREFIX,
                                             escape=True),
                      error_message='Checksum Range wasn´t avoided the second time')


def test_cyware_db_timeout(configure_sockets_environment,
                          connect_to_sockets_module,
                          pre_insert_packages,
                          pre_set_sync_info):
    """Check that effectively the socket is closed after timeout is reached"""
    cyware_db_send_sleep = 2
    command = 'agent 000 package get'
    receiver_sockets[0].send(command, size=True)

    # Waiting Cyware-DB to process command
    time.sleep(cyware_db_send_sleep)
