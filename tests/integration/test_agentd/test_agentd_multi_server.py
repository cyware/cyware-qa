...
brief: A Cyware cluster is a group of Cyware managers that work together to enhance the availability
...
How does this test work:
    ...
    - SIMULATOR_NUMBERS: Number of simulator to be instantiated, this should match cyware_conf.yaml
...
def restart_agentd():
    ...
    control_service('start', daemon="cyware-agentd", debug_mode=True)
...
@pytest.mark.skip(reason='https://github.com/cyware/cyware-qa/issues/3536')
def test_agentd_multi_server(add_hostnames, configure_authd_server, set_authd_id, clean_keys, configure_environment,
    ...
    parameters:
        ...
        - set_authd_id:
            type: fixture
            brief: Sets the agent id to '101' in the 'cyware-authd' simulated connection.
        - clean_keys:
            type: fixture
    ...
    assertions:
        - Agent without keys. Verify that all servers will refuse the connection to the 'cyware-remoted' daemon
        ...
        - Agent without keys. Verify that the first server only has enrollment available, and the third server
          only has the 'cyware-remoted' daemon available. The agent should enroll in the first server and
