...
brief: The 'cyware-agentd' program is the client-side daemon that communicates with the server.
       These tests will check if the content of the 'cyware-agentd' daemon statistics file is valid.
       The statistics files are documents that show real-time information about the Cyware environment.
...
from cyware_testing.agent import (CLIENT_KEYS_PATH, set_state_interval, callback_ack, callback_keepalive,
...
test_data_file = os.path.join(test_data_path, 'cyware_state_tests.yaml')
...
def test_agentd_state(configure_environment, test_case):
    '''
    description: Check that the statistics file 'cyware-agentd.state' is created automatically
    ...
    assertions:
        ...
        - Verify that the information stored in the 'cyware-agentd.state' statistics file
          is consistent with the connection status to the 'cyware-remoted' daemon.

    input_description: An external YAML file (cyware_conf.yaml) includes configuration settings for the agent.
                       Different test cases that are contained in an external YAML file (cyware_state_tests.yaml)
...
def wait_connect(update_position=False):
    ...
    Args:
        update_position (bool, optional): update position after reading.
                                          Defaults to False.
    """
    cyware_log_monitor.start(timeout=240,
