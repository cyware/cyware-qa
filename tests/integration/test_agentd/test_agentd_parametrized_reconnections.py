...
brief: The 'cyware-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'cyware-agentd' daemon behaves when there are delays
       between connection attempts to the 'cyware-remoted' daemon using TCP and UDP protocols.
...
def test_agentd_parametrized_reconnections(configure_authd_server, start_authd, stop_agent, set_keys,
    ...
    parameters:
        ...
        - start_authd:
            type: fixture
            brief: Enable the 'cyware-authd' daemon to accept connections and perform enrollments.
        - stop_agent:
            type: fixture
            brief: Stop Cyware's agent.
        - set_keys:
            type: fixture
    ...
    assertions:
        - Verify that when the 'cyware-agentd' daemon initializes, it connects to
          the 'cyware-remoted' daemon of the manager before reaching the maximum number of attempts.
