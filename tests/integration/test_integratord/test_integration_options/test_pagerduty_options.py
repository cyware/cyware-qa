...
brief: Integratord manages cyware integrations with other applications such as Slack, Pagerduty, Shuffle, Yara or
...
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/capabilities/virustotal-scan/integration.html
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/daemons/cyware-integratord.htm
...
def test_pagerduty_no_option_tag(configuration, metadata, set_cyware_configuration, truncate_monitored_files,
                                 configure_local_internal_options_module, restart_cyware_daemon_function,
    ...
    parameters:
        ...
        - restart_cyware_daemon_function:
            type: fixture
            brief: Restart cyware daemon before starting a test.
        - wait_for_start_module:
            type: fixture
    ...
    # Check integration is enabled
    evm.detect_integration_enabled(integration=metadata['integration'], file_monitor=cyware_monitor)

    # Check no options JSON file is detected
    evm.detect_options_json_file_does_not_exist(file_monitor=cyware_monitor)

    # Check the message is sent to the integration's server
    evm.get_message_sent(integration='PagerDuty', file_monitor=cyware_monitor)
...
def test_pagerduty_options(configuration, metadata, set_cyware_configuration, truncate_monitored_files,
                           configure_local_internal_options_module, restart_cyware_daemon_function,
                           wait_for_start_module):
    '''
    ...
    if not metadata['sends_message']:
        ...
        evm.detect_unable_to_run_integration(integration=metadata['integration'], file_monitor=cyware_monitor)
    else:
        # Verify that the message is sent
        message = evm.get_message_sent(integration='PagerDuty', file_monitor=cyware_monitor)
