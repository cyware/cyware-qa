...
brief: The 'cyware-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       tag and the Cyware API returns the same values for the configured 'localfile' section.
...
def check_configuration_age_valid(cfg):
    """Check if the Cyware module runs correctly and analyze the desired file.
...
def check_configuration_age_invalid(cfg):
    """Check if the Cyware fails because the invalid age configuration value.
...
def test_configuration_age(get_configuration, configure_environment):
    '''
    description: Check if the 'cyware-logcollector' daemon detects invalid configurations for the 'age' tag.
    ...
    input_description: A configuration template (test_basic_configuration_age) is contained in an external YAML file
                       (cyware_basic_configuration.yaml). That template is combined with different test cases defined
    ...
    if cfg['valid_value']:
        ...
        if cfg['age'] in problematic_values:
            pytest.xfail("Logcollector accepts invalid values: https://github.com/cyware/cyware/issues/8158")
        else:
            if sys.platform == 'win32':
