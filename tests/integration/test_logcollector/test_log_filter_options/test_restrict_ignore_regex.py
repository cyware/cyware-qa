...
from cyware_testing.tools.services import get_service
...
def test_restrict_ignore_regex_values(configuration, metadata, new_file_path, create_file, truncate_monitored_files,
    ...
    expected_output:
        - r".*cyware-logcollector.*Analizing file: '{file}'.*"
        - r".*cyware-logcollector.*DEBUG: Reading syslog '{message}'.*"
        - r".*cyware-logcollector.*DEBUG: Ignoring the log line '{message}' due to {tag} config: '{regex}'"
    '''
