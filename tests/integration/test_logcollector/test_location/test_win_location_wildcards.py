...
brief: The 'cyware-logcollector' daemon monitors configured files and commands for new log messages. Secifically, these
...
import re
import pytest
from cyware_testing.modules import TIER1, WINDOWS
...
from cyware_testing.modules.logcollector import event_monitor as evm
from cyware_testing.modules import logcollector as lc
...
def test_win_location_wildcards(configuration, metadata, folder_path, file_list, create_files_in_folder,
                                truncate_monitored_files, set_cyware_configuration,
    ...
    expected_output:
        - r".*cyware-agent.*expand_win32_wildcards.*DEBUG: No file/folder that matches {regex}"
        - r".*cyware-agent.*check_pattern_expand.*New file that matches the '{file_path}' pattern: '(.*)'"
        - r".*cyware-agent.*Analizing file: '{file}'.*"
        - r".*cyware-agent.*DEBUG: Reading syslog '{message}'.*"
    '''
