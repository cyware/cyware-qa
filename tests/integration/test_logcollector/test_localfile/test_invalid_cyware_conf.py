...
brief: The 'cyware-logcollector' daemon monitors configured files and commands for new log messages. When Cyware is
       configured incorrectly then a configuration error is displayed in the Cyware's log, and Cyware does not start (if
...
from cyware_testing.tools import LOGCOLLECTOR_DAEMON, LOG_FILE_PATH
...
from cyware_testing.fim import callback_configuration_error
from cyware_testing.logcollector import LOG_COLLECTOR_GLOBAL_TIMEOUT, callback_missing_element_error
...
parameters = [
    {'LOCATION': os.path.join(temp_dir, 'cyware-testing', files[0]), 'LOG_FORMAT': None},
...
configurations_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data', 'invalid_cyware_conf.yaml')
...
def test_invalid_cyware_conf(get_files_list, create_file_structure_module, get_configuration, remove_empty_options,
    ...
    description: Check if the expected message is present in the ossec.log when an invalid <localfile> configuration is
                 set and if Cyware refuses to restart.
    ...
    cyware_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback_missing_element_error,
                            error_message='Did not receive the expected "ERROR: ...: Missing ... element.')

    cyware_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback_configuration_error,
