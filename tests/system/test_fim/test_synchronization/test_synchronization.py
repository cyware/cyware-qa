...
os_version:
    ...
    - Red Hat 6
references:
    - https://github.com/cyware/cyware-qa/issues/2434
tags:
    - fim_synchronization
"""
...
from cyware_testing.tools.system import HostManager, clean_environment
from cyware_testing.tools import CYWARE_LOGS_PATH
from cyware_testing.fim import create_folder_file, query_db
...
enviroment_files = [('cyware-manager', os.path.join(CYWARE_LOGS_PATH, 'ossec.log')),
                    ('cyware-agent1', os.path.join(CYWARE_LOGS_PATH, 'ossec.log'))]


@pytest.mark.parametrize('host', ['cyware-agent1', 'cyware-manager'])
...
def test_synchronization(folder_path, case, host):
    ...
    try:
        ...
        host_manager.run_command('cyware-agent1', f'rm -rf {folder_path}')

    clean_environment(host_manager, enviroment_files)

    # Stop host
    host_manager.run_command(host, '/var/ossec/bin/cyware-control stop')

    if (case == 'add'):
        host_manager.run_command('cyware-agent1', f'touch {folder_path}/{folder_path}.txt')

    elif (case == 'modify'):
        host_manager.modify_file_content(host='cyware-agent1', path=folder_path, content=folder_path)
    ...
    # Start host
    host_manager.run_command(host, '/var/ossec/bin/cyware-control start')

    if (host == 'cyware-manager'):
