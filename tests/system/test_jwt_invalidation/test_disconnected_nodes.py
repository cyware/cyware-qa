...
def control_cyware_services(node, state=None):
    """Control Cyware services with `command` instead of `service` due to incompatibility."""
    host_manager.get_host(node).ansible('command', f'service cyware-manager {state}', check=False)
    host_manager.get_host(node).ansible('command', f'service cyware-api {state}', check=False)
    if 'start' in state:
        time.sleep(10)
...
def clean_environment():
    ...
    token = host_manager.get_api_token('cyware-master')
    response = host_manager.make_api_call('cyware-master', method='DELETE',
    ...
    for host in test_hosts[1:]:
        control_cyware_services(host, state='restart')
...
def test_create_user_when_node_is_disconnected(set_default_api_conf, clean_environment):
    ...
    control_cyware_services('cyware-worker1', state='stop')
    control_cyware_services('cyware-worker2', state='stop')

    # Get token in the master node
    master_token = host_manager.get_api_token('cyware-master')

    # Create user in the master node
    test_user = 'NewTestUser'
    test_pass = 'NewPassword1*'
    response = host_manager.make_api_call('cyware-master', method='POST', endpoint='/security/users',
    ...
    control_cyware_services('cyware-worker1', state='start')
    host_manager.get_api_token('cyware-worker1', user=test_user, password=test_pass)
    ...
    control_cyware_services('cyware-worker2', state='start')
    # 'KeyError' since the `get_api_token` tries to return `response['json']['token']`
    with pytest.raises(KeyError):
        host_manager.get_api_token('cyware-worker2', user=test_user, password=test_pass)
