...
test_hosts = ['cyware-master', 'cyware-worker1']
...
@pytest.mark.parametrize('login_endpoint, host, old_password, new_password', [
    ...
    ({}, 'cyware-master', test_passw, 'Newpass1*'),
    ({}, 'cyware-worker1', 'Newpass1*', 'Newpass2*'),
    # Auth context login
    ({"auth_context": {"username": "testing"}}, 'cyware-master', 'Newpass2*', 'Newpass1*'),
    ({"auth_context": {"username": "testing"}}, 'cyware-worker1', 'Newpass1*', 'Newpass2*')
])
