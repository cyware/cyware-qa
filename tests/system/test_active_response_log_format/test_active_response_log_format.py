...
os_version:
    ...
    - Red Hat 6
references:
    - https://github.com/cyware/cyware/issues/10858
tags:
    - active_response
"""
...
testinfra_hosts = ["cyware-manager", "cyware-agent1", "cyware-agent2", "cyware-agent3"]
...
@pytest.mark.parametrize('cyware_agent, message_file', [('cyware-agent1', messages_files[0]),
                                                       ('cyware-agent2', messages_files[1]),
                                                       ('cyware-agent3', messages_files[2])])
def test_active_response_log_format(cyware_agent, message_file):
    """
    ...
    enviroment_files = [(cyware_agent, os.path.join(CYWARE_LOGS_PATH, 'ossec.log')),
                        (cyware_agent, os.path.join(CYWARE_LOGS_PATH, 'active-responses.log'))]

    clean_environment(host_manager, enviroment_files)

    # Add log message to agent monitored source
    host_manager.modify_file_content(host=cyware_agent, path=log_path, content=log_sample)
    ...
    host_manager.modify_file_content(host=cyware_agent, path=log_path, content="")
