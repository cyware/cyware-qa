'''
copyright: Copyright (C) 2023-2024, KhulnaSoft Ltd.
           Created by Cyware, Inc. <info@khulnasoft.com>.
           This program is free software; you can redistribute it and/or modify it under the terms of GPLv2
type: system
brief: The agent-auth program is the client application used along with 'cyware-authd' to automatically add agents to a
       Cyware manager. These tests will check if the 'cyware-authd' daemon processes registration messages correctly.
tier: 0
modules:
    - enrollment
components:
...
os_version:
    - Debian Buster
references:
    - https://documentation.cyware.khulnasoft.com/current/user-manual/reference/tools/agent-auth.html
tags:
    - authd
'''
...
from cyware_testing.tools.utils import format_ipv6_long


pytestmark = [pytest.mark.basic_environment_env]

# Hosts
testinfra_hosts = ["cyware-manager", "cyware-agent1"]
...
def clean_environment():
    ...
    agent_id = host_manager.run_command('cyware-manager', f'cut -c 1-3 {CYWARE_PATH}/etc/client.keys')
    host_manager.run_command('cyware-manager', f"{CYWARE_PATH}/bin/manage_agents -r {agent_id}")
    host_manager.clear_file(host='cyware-manager', file_path=os.path.join(CYWARE_PATH, 'etc', 'client.keys'))
...
def get_ip_directions():
    ...
    manager_network = host_manager.get_host_ip('cyware-manager', 'eth0')
    agent_network = host_manager.get_host_ip('cyware-agent1', 'eth0')
...
def configure_network(test_case):
    ...
    for configuration in test_case['test_case']:
        ...
        if 'ipv6' in configuration['manager_network']:
            host_manager.run_command('cyware-manager', 'ip -4 addr flush dev eth0')
        elif 'ipv4' in configuration['manager_network']:
            host_manager.run_command('cyware-manager', 'ip -6 addr flush dev eth0')

        # Agent network configuration
        if 'ipv6' in configuration['agent_network']:
            host_manager.run_command('cyware-agent1', 'ip -4 addr flush dev eth0')

        elif 'ipv4' in configuration['agent_network']:
            host_manager.run_command('cyware-agent1', 'ip -6 addr flush dev eth0')
    ...
    for configuration in test_case['test_case']:
        ...
        if 'ipv6' in configuration['manager_network']:
            host_manager.run_command('cyware-manager', f"ip addr add {network['manager_network'][0]} dev eth0")
            host_manager.run_command('cyware-manager', 'ip route add 172.24.27.0/24 via 0.0.0.0 dev eth0')
        elif 'ipv4' in configuration['manager_network']:
            host_manager.run_command('cyware-manager', f"ip addr add {network['manager_network'][1]} dev eth0")
            host_manager.run_command('cyware-manager', f"ip addr add {network['manager_network'][2]} dev eth0")

        # Restore agent network configuration
        if 'ipv6' in configuration['agent_network']:
            host_manager.run_command('cyware-agent1', f"ip addr add {network['agent_network'][0]} dev eth0")
            host_manager.run_command('cyware-agent1', 'ip route add 172.24.27.0/24 via 0.0.0.0 dev eth0')
        elif 'ipv4' in configuration['agent_network']:
            host_manager.run_command('cyware-agent1', f"ip addr add {network['agent_network'][1]} dev eth0")
            host_manager.run_command('cyware-agent1', f"ip addr add {network['manager_network'][2]} dev eth0")
...
def modify_ip_address_conf(test_case):
    ...
    for configuration in test_case['test_case']:
        ...
        if 'ipv4' in configuration['ip_type']:
            ...
            expected_message_manager_ip = 'cyware-manager'
        ...
        host_manager.modify_file_content(host='cyware-agent1', path='/var/ossec/etc/ossec.conf',
...
def test_agent_auth(test_case, get_ip_directions, configure_network, modify_ip_address_conf, clean_environment):
    ...
    host_manager.clear_file(host='cyware-manager', file_path=os.path.join(CYWARE_LOGS_PATH, 'ossec.log'))
    host_manager.control_service(host='cyware-manager', service='cyware', state="restarted")

    # Start the agent enrollment process using agent-auth
    for configuration in test_case['test_case']:
        if 'ipv4' in configuration['ip_type']:
            host_manager.run_command('cyware-agent1', f"{CYWARE_PATH}/bin/agent-auth -m {network['manager_network'][0]}")
        elif 'ipv6' in configuration['ip_type']:
            host_manager.run_command('cyware-agent1', f"{CYWARE_PATH}/bin/agent-auth -m {network['manager_network'][1]}")
        else:
            host_manager.run_command('cyware-agent1', f"{CYWARE_PATH}/bin/agent-auth -m cyware-manager")
    ...
    host_manager.control_service(host='cyware-agent1', service='cyware', state="started")

    # Make sure the agent's and manager's client.keys have the same keys
    agent_client_keys = host_manager.get_file_content('cyware-agent1', os.path.join(CYWARE_PATH, 'etc', 'client.keys'))
    manager_client_keys = host_manager.get_file_content('cyware-agent1', os.path.join(CYWARE_PATH, 'etc', 'client.keys'))
    assert agent_client_keys == manager_client_keys

    # Check if the agent is active
    agent_id = host_manager.run_command('cyware-manager', f'cut -c 1-3 {CYWARE_PATH}/etc/client.keys')
    sleep(wait_agent_start)
    agent_info = host_manager.run_command('cyware-manager', f'{CYWARE_PATH}/bin/agent_control -i {agent_id}')
