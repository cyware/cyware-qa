...
type: system
brief: Cyware includes a registration process that provides the user with an automated mechanism to enroll agents with
...
from cyware_testing.tools.file import read_file, read_yaml, write_file
...
def clean_environment():
    ...
    agent_ids = host_manager.run_command('cyware-manager', f'cut -c 1-3 {CYWARE_PATH}/etc/client.keys').split()
...
def configure_network(test_case):
    ...
    if 'ipv6' in test_case['agent_network']:
        ...
        host_manager.run_command('cyware-agent1', f"ip addr add {network['agent_network'][2]} dev eth0")
...
def test_agent_enrollment(test_case, get_ip_directions, configure_network, modify_ip_address_conf, clean_environment):
    ...
    host_manager.control_service(host='cyware-agent1', service='cyware', state="restarted")
