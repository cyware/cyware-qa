...
from cyware_testing.tools import CYWARE_PATH, PYTHON_PATH, CYWARE_LOGS_PATH
...
# Hosts
test_hosts = ['cyware-master', 'cyware-worker1', 'cyware-worker2']
...
directories_to_create = [os.path.join(CYWARE_PATH, 'etc', 'shared', 'test_group'),
                         os.path.join(CYWARE_PATH, 'var', 'multigroups', 'test_dir')]

# Files that, after created in the master node, should be present in all nodes.
files_to_sync = [os.path.join(CYWARE_PATH, 'etc', 'lists', 'test_file'),
                 os.path.join(CYWARE_PATH, 'etc', 'rules', 'test_file'),
                 os.path.join(CYWARE_PATH, 'etc', 'decoders', 'test_file'),
...
files_not_to_sync = [os.path.join(CYWARE_PATH, 'etc', 'test_file'),
                     os.path.join(CYWARE_PATH, 'etc', 'lists', 'ar.conf'),
                     os.path.join(CYWARE_PATH, 'etc', 'lists', 'ossec.conf'),
                     os.path.join(CYWARE_PATH, 'etc', 'lists', 'test.tmp'),
                     os.path.join(CYWARE_PATH, 'etc', 'lists', 'test.lock'),
                     os.path.join(CYWARE_PATH, 'etc', 'lists', 'test.swp'),
                     os.path.join(directories_to_create[1], 'test_file')]

# Directories where to create big files.
tmp_size_test_path = os.path.join(CYWARE_PATH, 'tmp')
dst_size_test_path = os.path.join(CYWARE_PATH, 'etc', 'rules')
...
def create_file(host, path, size):
    ...
    host_manager.run_command(host, f"chown cyware:cyware {path}")
...
def update_cluster_json():
    ...
    for host in test_hosts:
        ...
        host_manager.control_service(host=host, service='cyware', state='restarted')
    ...
    for host in backup_json:
        ...
        host_manager.control_service(host=host, service='cyware-manager', state='restarted')
...
def test_missing_file(clean_files):
    ...
    for subdir in directories_to_create:
        ...
        host_manager.run_command(test_hosts[0], f"chown cyware:cyware {subdir}")
    ...
    for file in files_to_sync + files_not_to_sync + [agent_conf_file]:
        ...
        host_manager.run_command(test_hosts[0], f"chown cyware:cyware {file}")
