...
test_infra_agents = ['cyware-agent2', 'cyware-agent3']
master_host = 'cyware-master'
...
script_path = os.path.join(re.sub(r'^.*?cyware-qa', '/cyware-qa', local_path), '../utils/get_wdb_agent.py')
...
label = 'test_label'
modified_agent = 'cyware-agent2'
deleted_agent = 'cyware-agent3'
while_time = 5
time_to_sync = 21
...
def clean_cluster_logs():
    ...
    for host in testinfra_hosts:
        host_manager.clear_file(host=host, file_path=os.path.join(CYWARE_LOGS_PATH, 'cluster.log'))
        host_manager.clear_file(host=host, file_path=os.path.join(CYWARE_LOGS_PATH, 'ossec.log'))

        # Its required to restart each node after clearing the log files
        host_manager.get_host(host).ansible('command', 'service cyware-manager restart', check=False)

    for agent in test_infra_agents:
        host_manager.clear_file(host=agent, file_path=os.path.join(CYWARE_LOGS_PATH, 'ossec.log'))
        host_manager.get_host(agent).ansible('command', 'service cyware-agent restart', check=False)


@pytest.fixture(scope='function')
def remove_labels():
    """Remove any label set to the modified cyware-agent and restart it to apply the new config."""
    yield
    host_manager.add_block_to_file(host=modified_agent, path=f"{CYWARE_PATH}/etc/ossec.conf",
                                   after='</client>', before='<client_buffer>', replace=os.linesep)
    host_manager.get_host(modified_agent).ansible('command', 'service cyware-agent restart', check=False)
...
def test_agent_info_sync(clean_cluster_logs, remove_labels):
    ...
    modified_agent_id = host_manager.run_command(master_host,
                                                 f"{CYWARE_PATH}/framework/python/bin/python3 "
...
def test_agent_info_sync_remove_agent(clean_cluster_logs):
    ...
    agent_list = host_manager.run_command('cyware-worker2',
                                          f"{CYWARE_PATH}/framework/python/bin/python3 "
                                          f"{script_path} \"{queries[2]}\"")

    assert deleted_agent in agent_list, f"{deleted_agent} was not found in cyware-worker2\'s global.db"
    ...
    host_manager.get_host(deleted_agent).ansible('command', 'service cyware-agent stop', check=False)
    ...
    host_manager.get_host(deleted_agent).ansible('command', 'service cyware-agent restart', check=False)
    check_agent_status('active', master_token, deleted_agent)
