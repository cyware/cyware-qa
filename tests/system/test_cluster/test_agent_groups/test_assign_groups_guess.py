...
os_version:
    ...
    - Red Hat 6
references:
    - https://github.com/cyware/cyware-qa/issues/2513
tags:
    - cluster
"""
import os
...
from cyware_testing.tools import CYWARE_PATH, CYWARE_LOCAL_INTERNAL_OPTIONS


pytestmark = [pytest.mark.cluster, pytest.mark.enrollment_cluster_env]

# Hosts
test_infra_managers = ['cyware-master', 'cyware-worker1', 'cyware-worker2']
test_infra_agents = ['cyware-agent1', 'cyware-agent2']
...
def modify_local_internal_options(status_guess_agent_group):
    ...
    local_internal_options = {'cyware-master': [{'name': 'remoted.guess_agent_group', 'value': '0'}],
                              'cyware-worker1': [{'name': 'remoted.debug', 'value': '2'}, {'name': 'authd.debug',
                                                                                          'value': '2'}]}
    """
    ...
    backup_local_internal_options = host_manager.get_file_content(test_infra_managers[0], CYWARE_LOCAL_INTERNAL_OPTIONS)
    ...
    host_manager.modify_file_content(test_infra_managers[0], CYWARE_LOCAL_INTERNAL_OPTIONS,
                                     backup_local_internal_options)


# Tests
@pytest.mark.parametrize('status_guess_agent_group', ['0', '1'])
@pytest.mark.parametrize('target_node', ['cyware-master', 'cyware-worker1'])
def test_guess_single_group(target_node, status_guess_agent_group, clean_environment, modify_local_internal_options):
    ...
    parameters:
        ...
        - clean_enviroment:
            type: Fixture
            brief: Reset the cyware log files at the start of the test. Remove all registered agents from master.
    ...
    try:
        ...
        messages_path = master_messages_path if target_node == 'cyware-master' else worker_messages_path
