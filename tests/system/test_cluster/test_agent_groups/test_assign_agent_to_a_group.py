...
os_version:
    ...
    - Red Hat 6
references:
    - https://github.com/cyware/cyware-qa/issues/2507
tags:
    - cluster
"""
import os
...
@pytest.mark.parametrize("agent_target", ["cyware-master", "cyware-worker1"])
def test_assign_agent_to_a_group(agent_target, initial_status, clean_environment):
    ...
    if (initial_status == AGENT_STATUS_DISCONNECTED):
        host_manager.control_service(host='cyware-agent1', service=CYWARE_SERVICE_PREFIX, state=CYWARE_SERVICES_STOPPED)
        time.sleep(timeout)
    ...
    try:
        ...
        assign_agent_to_new_group('cyware-master', test_group, agent_id, host_manager)
        ...
        delete_agent_group('cyware-master', test_group, host_manager)
