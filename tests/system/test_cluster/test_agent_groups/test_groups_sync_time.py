...
brief: This tests check that when a cluster has a series of agents with groups assigned, when an agent has it's
        group changed by a Cyware-DB command, the cluster updates it's information.
tier: 2
modules:
    - cluster
components:
...
references:
    ...
    - https://documentation.cyware.khulnasoft.com/current/user-manual/registering/command-line-registration.html
...
for x in range(agents_in_cluster):
    test_infra_agents.append("cyware-agent" + str(x+1))
...
def test_agent_groups_sync_time(agent_host, clean_environment):
    ...
    while time.time() < end_time:
        if active_agent < agents_in_cluster:
            host_manager.run_command(test_infra_agents[active_agent], f'{CYWARE_PATH}/bin/cyware-control start')
