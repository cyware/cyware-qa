...
type: system
brief: Cyware manager handles agent groups.
...
from cyware_testing import T_025, T_1, T_5, T_10
...
from system import (assign_agent_to_new_group, create_new_agent_group, delete_agent_group, execute_wdb_query,
                    restart_cluster)
from cyware_testing.tools.configuration import get_test_cases_data
from system.test_cluster.test_agent_groups.common import register_agent

pytestmark = [pytest.mark.cluster, pytest.mark.enrollment_cluster_env]

test_infra_hosts = ['cyware-master', 'cyware-worker1', 'cyware-worker2', 'cyware-agent1', 'cyware-agent2']
...
@pytest.mark.parametrize('target_node', ['cyware-master', 'cyware-worker1', 'cyware-worker2'])
...
def test_group_sync_status(metadata, target_node, clean_environment, group_creation_and_assignation,
    ...
    '''
    description: Delete a group folder in cyware server cluster and check group_sync status in 2 times.
    ...
    assertions:
        ...
        - Verify same conditions creating and assigning groups from all cyware-manager clusters (Master and Workers)
