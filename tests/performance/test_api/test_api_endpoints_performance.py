...
test_data = safe_load(open(join(dirname(realpath(__file__)), 'data', 'cyware_api_endpoints_performance.yaml')))
...
xfailed_items = {
    '/active-response': {'message': 'Agent simulator not handling active-response messages: '
                                    'https://github.com/cyware/cyware-qa/issues/1266',
                         'method': 'put'},
    '/agents/group': {'message': 'Investigate performance issues with PUT /agents/group API endpoint: '
                                 'https://github.com/cyware/cyware/issues/13872',
                      'method': 'put'}
}
