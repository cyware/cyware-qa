...
The Cyware architecture is based on agent, running on the monitored endpoints, that forward security data to a central server. The central server decodes and analyzes the incoming information and passes the results along to the Cyware indexer for indexing and storage. The Cyware indexer cluster is a collection of one or more nodes that communicate with each other to perform read and write operations on indices.

The diagram below represents the Cyware components and data flow.

![cyware-data-flow](https://documentation.cyware.khulnasoft.com/current/_images/data-flow1.png)
...
Our newest integration tests are located in `cyware-qa/tests/end_to_end/test_basic_cases`:
...
rootdir: /home/juliamagan/Desktop/QA/cyware-qa
