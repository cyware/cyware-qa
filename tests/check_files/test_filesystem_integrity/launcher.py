...
from cyware_testing.qa_ctl.configuration.config_instance import ConfigInstance
from cyware_testing.qa_ctl.configuration.config_generator import QACTLConfigGenerator
from cyware_testing.qa_ctl.provisioning.ansible import playbook_generator
from cyware_testing.tools.s3_package import get_production_package_url, get_last_production_package_url
from cyware_testing.tools.time import get_current_timestamp
from cyware_testing.tools import file
from cyware_testing.qa_ctl.provisioning import local_actions
from cyware_testing.tools import github_checks
...
TMP_FILES = os.path.join(gettempdir(), 'cyware_check_files')
CYWARE_QA_FILES = os.path.join(TMP_FILES, 'cyware-qa')
CHECK_FILES_TEST_PATH = os.path.join(CYWARE_QA_FILES, 'tests', 'check_files', 'test_filesystem_integrity')
...
def get_parameters():
    ...
    parser.add_argument('--action', '-a', type=str, action='store', required=False, dest='test_action',
                        ...
                        help='Cyware action to be carried out to check the check-files')
    ...
    parser.add_argument('--version', '-v', type=str, action='store', required=False, dest='cyware_version',
                        help='Cyware installation and tests version.')
    ...
    parser.add_argument('--qa-branch', type=str, action='store', required=False, dest='qa_branch', default='master',
                        help='Set a custom cyware-qa branch to download and run the tests files')

    parser.add_argument('--target', '-t', type=str, action='store', required=False, dest='cyware_target',
                        choices=['manager', 'agent'], default='manager', help='Cyware test target. manager or agent')
...
def set_environment(parameters):
    ...
    # Download cyware-qa repository to launch the check-files test files.
    local_actions.download_local_cyware_qa_repository(branch=parameters.qa_branch, path=TMP_FILES)
    test_build_files.append(CYWARE_QA_FILES)
...
def validate_parameters(parameters):
    ...
    if not github_checks.branch_exists(parameters.qa_branch, repository=CYWARE_QA_REPO):
        raise QAValueError(f"{parameters.qa_branch} branch does not exist in Cyware QA repository.",
                           logger.error, QACTL_LOGGER)

    # Check version parameter
    if parameters.cyware_version and len((parameters.cyware_version).split('.')) != 3:
        raise QAValueError(f"Version parameter has to be in format x.y.z. You entered {parameters.cyware_version}",
                           logger.error, QACTL_LOGGER)

    # Check if Cyware has the specified version
    if parameters.cyware_version and not github_checks.version_is_released(parameters.cyware_version):
        raise QAValueError(f"The cyware {parameters.cyware_version} version has not been released. Enter a right "
...
def generate_test_playbooks(parameters, local_checkfiles_pre_data_path, local_checkfiles_post_data_path):
    ...
    """
    playbooks_info = {}
    package_url = get_production_package_url(parameters.cyware_target, parameters.os_system, parameters.cyware_version) \
        if parameters.cyware_version else get_last_production_package_url(parameters.cyware_target, parameters.os_system)
    package_name = os.path.split(package_url)[1]

    check_files_tool_url = f"https://raw.githubusercontent.com/cyware/cyware-qa/{parameters.qa_branch}/deps/" \
                           'cyware_testing/cyware_testing/scripts/check_files.py'
    os_platform = 'linux'
    ...
    # Playbook parameters
    cyware_install_playbook_parameters = {'cyware_target': parameters.cyware_target, 'package_name': package_name,
    ...
    upgrade_cyware_playbook_parameters = {'package_name': package_name,  'package_url': package_url,
                                         'package_destination': package_destination, 'os_system': parameters.os_system,
                                         'os_platform': os_platform}

    uninstall_cyware_playbook_parameters = {'cyware_target': parameters.cyware_target, 'os_system': parameters.os_system,
                                           'os_platform': os_platform}
    ...
    if parameters.test_action == 'install':
        # 1. - Install Cyware on remote host
        # 2. - Wait 30 seconds before running check-files tool
        playbooks_info.update({'install_cyware': playbook_generator.install_cyware(**cyware_install_playbook_parameters)})

    elif parameters.test_action == 'upgrade':
        cyware_pre_version = '4.2.0'
        install_package_url = get_production_package_url(parameters.cyware_target, parameters.os_system,
                                                         cyware_pre_version)
        install_package_name = os.path.split(install_package_url)[1]
        upgrade_package_url = get_last_production_package_url(parameters.cyware_target, parameters.os_system)
        upgrade_package_name = os.path.split(upgrade_package_url)[1]
        cyware_install_playbook_parameters.update({'package_url': install_package_url,
                                                  'package_name': install_package_name})
        upgrade_cyware_playbook_parameters.update({'package_url': upgrade_package_url,
                                                  'package_name': upgrade_package_name})
        # 1. - Install Cyware on remote host
        # 2. - Wait 30 seconds before upgrading Cyware
        # 3. - Upgrade Cyware on remote host
        # 4. - Wait 30 seconds before running check-files tool
        playbooks_info.update({'install_cyware': playbook_generator.install_cyware(**cyware_install_playbook_parameters)})
        playbooks_info.update({'waiting_time_before_upgrading_cyware': playbook_generator.wait_seconds(30)})
        playbooks_info.update({'upgrade_cyware': playbook_generator.upgrade_cyware(**upgrade_cyware_playbook_parameters)})

    elif parameters.test_action == 'uninstall':
        # 1. - Install Cyware on remote host
        # 2. - Wait 30 seconds before uninstalling Cyware
        # 3. - Uninstall Cyware on remote host
        playbooks_info.update({'install_cyware': playbook_generator.install_cyware(**cyware_install_playbook_parameters)})
        playbooks_info.update({'waiting_time_before_upgrading_cyware': playbook_generator.wait_seconds(30)})
        playbooks_info.update({'uninstall_cyware':
                               playbook_generator.uninstall_cyware(**uninstall_cyware_playbook_parameters)})
