...
from cyware_testing import global_parameters, logger, REGULAR, LOG_FILE_PATH, CYWARE_PATH
from cyware_testing.tools.file import create_file, modify_file_content, delete_file, generate_string
...
from cyware_testing.modules.fim import event_monitor as ev
from cyware_testing.modules.fim.classes import CustomValidator, EventChecker, RegistryEventChecker
...
def get_sync_msgs(timeout, new_data=True):
    ...
    events = []
    if new_data:
        cyware_log_monitor.start(timeout=timeout,
    ...
    for _ in range(0, fim.MAX_EVENTS_VALUE):
        try:
            sync_event = cyware_log_monitor.start(timeout=global_parameters.default_timeout,
