...
class AgentDeployment(CywareDeployment):
    """Deploy Cyware agent with all the elements needed, set from the configuration file
    ...
    def install(self):
        """Child method to install Cyware in agent
    ...
    def register_agent(self):
        """Set the manager ip in the ossec.conf, and restart Cyware agent to let autoenrollment register it.
    ...
    def health_check(self):
        ...
        tasks_list.append(AnsibleTask({
            ...
            'register': 'status',
            'become': True,
            'failed_when': ['"cyware-agentd" not in status.stdout',
                            '"cyware-execd" not in status.stdout']
        }))
