...
from cyware_testing.qa_ctl.provisioning.ansible.ansible_playbook import AnsiblePlaybook
...
PLAYBOOSK_PATH = os.path.join(gettempdir(), 'cyware_playbooks')
...
def install_cyware(cyware_target, package_name, package_url, package_destination, os_system, os_platform,
                  playbook_parameters=None):
    """Generate the playbook to install Cyware.

    Args:
        cyware_target (str): Cyware target [manager or agent].
    ...
    if os_platform == 'linux':
        if PACKAGE_MANAGER[_os_system] == 'RPM':
            tasks = _install_cyware_rpm(package_name, package_url, package_destination, cyware_target)
        elif PACKAGE_MANAGER[_os_system] == 'DEB':
            tasks = _install_cyware_deb(package_name, package_url, package_destination, cyware_target)
        else:
    ...
    parameters.update({'name': 'install_cyware', 'tasks_list': tasks})

    return _build_playbook(parameters)


def upgrade_cyware(package_name, package_url, package_destination, os_system, os_platform, playbook_parameters=None):
    """Generate the playbook to upgrade Cyware.
    ...
    if os_platform == 'linux':
        if PACKAGE_MANAGER[_os_system] == 'RPM':
            tasks = _upgrade_cyware_rpm(package_name, package_url, package_destination)
        elif PACKAGE_MANAGER[_os_system] == 'DEB':
            tasks = _upgrade_cyware_deb(package_name, package_url, package_destination)
        else:
            raise ValueError(f"{os_system} is not supported in PlaybookGenerator")
    else:
        raise ValueError(f"{os_platform} is not supported in PlaybookGenerator")
    ...
    parameters = dict(**playbook_parameters) if playbook_parameters else {}
    parameters.update({'name': 'upgrade_cyware', 'tasks_list': tasks})

    return _build_playbook(parameters)


def uninstall_cyware(cyware_target, os_system, os_platform, playbook_parameters=None):
    """Generate the playbook to uninstall Cyware.
    ...
    if os_platform == 'linux':
        if PACKAGE_MANAGER[_os_system] == 'RPM':
            tasks = _uninstall_cyware_rpm(cyware_target)
        elif PACKAGE_MANAGER[_os_system] == 'DEB':
            tasks = _uninstall_cyware_deb(cyware_target)
        else:
            raise ValueError(f"{os_system} is not supported in PlaybookGenerator")
    else:
        raise ValueError(f"{os_platform} is not supported in PlaybookGenerator")
    ...
    parameters = dict(**playbook_parameters) if playbook_parameters else {}
    parameters.update({'name': 'uninstall_cyware', 'tasks_list': tasks})
...
def _download_cyware_package(package_url, package_destination):
    ...
    Returns:
        list(AnsibleTask): Ansible tasks to download the cyware package.
    """
    return [
        AnsibleTask({
            'name': 'Download Cyware package',
            'get_url': {
                'url': package_url,
...
def _start_cyware_manager_systemd_service():
    """"Ansible tasks to start the cyware-manager service with systemd.
    ...
    return [
        AnsibleTask({
            'name': 'Start Cyware Manager service with systemd',
            'become': True,
...
def _start_cyware_agent_systemd_service():
    """Ansible tasks to start the cyware-agent service with systemd.
    ...
    return [
        AnsibleTask({
            'name': 'Start Cyware Agent service with systemd',
            'become': True,
            'ansible.builtin.systemd': {
              'state': 'started',
              'name': 'cyware-agent'
...
def _stop_cyware_manager_systemd_service():
    """Ansible tasks to stop the cyware-manager service with systemd.
...
def _stop_cyware_agent_systemd_service():
    """Ansible tasks to stop the cyware-agent service with systemd.
...
def _start_cyware_control_service(cyware_target):
    """Ansible tasks to start the cyware-agent or cyware-manager using cyware-control.
    ...
    return [
        AnsibleTask({
            'name': f"Start Cyware {cyware_target} service with systemd",
            'become': True,
            'shell': '/var/ossec/bin/cyware-control start',
        })
    ]


def _stop_cyware_control_service(cyware_target):
    """Ansible tasks to stop the cyware-agent or cyware-manager using cyware-control.
    ...
    return [
        AnsibleTask({
            'name': f"Stop Cyware {cyware_target} service with systemd",
            'become': True,
            'shell': '/var/ossec/bin/cyware-control stop',
        })
    ]


def _install_cyware_rpm(package_name, package_url, package_destination, cyware_target):
    """Ansible tasks to install a cyware RPM package.
    ...
    tasks.extend(_download_cyware_package(package_url, package_destination))
    tasks.append(
        AnsibleTask({
            'name': 'Install Cyware RPM package',
            'become': True,
    ...
    if 'manager' in cyware_target:
        tasks.extend(_start_cyware_manager_systemd_service())
    elif 'agent' in cyware_target:
        tasks.extend(_start_cyware_agent_systemd_service())

    return tasks


def _install_cyware_deb(package_name, package_url, package_destination, cyware_target):
    """Ansible tasks to install a cyware DEB package.
    ...
    tasks.append(
        AnsibleTask({
            'name': 'Install Cyware DEB package',
            'become': True,
...
def _upgrade_cyware_rpm(package_name, package_url, package_destination):
    """Ansible tasks to upgrade a cyware RPM package.
    ...
    tasks.append(
        AnsibleTask({
            'name': 'Upgrade cyware RPM package',
            'become': True,
            'shell': f"yum install -y {package_name}",
            'args': {
                'chdir': package_destination
...
def _upgrade_cyware_deb(package_name, package_url, package_destination):
    """Ansible tasks to upgrade a cyware DEB package.
    ...
    tasks.append(
        AnsibleTask({
            'name': 'Upgrade cyware DEB package',
            'become': True,
            'apt': {
...
def _uninstall_cyware_rpm(cyware_target):
    """Ansible tasks to uninstall a cyware RPM package.
    ...
    tasks = [
        AnsibleTask({
            'name': 'Uninstall cyware RPM package',
            'become': True,
            'shell': f"yum remove -y cyware-{cyware_target}",
        })
    ]
...
def _uninstall_cyware_deb(cyware_target):
    """Ansible tasks to uninstall a cyware DEB package.
    ...
    return [
        AnsibleTask({
            'name': 'Uninstall cyware DEB package',
            'become': True,
            'apt': {
                'name': f"cyware-{cyware_target}",
                'state': 'absent',
                'purge': True
            }
        })
