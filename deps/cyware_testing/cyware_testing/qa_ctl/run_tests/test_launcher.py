...
class TestLauncher:
    ...
    ALL_DEBUG_OPTIONS = ["syscheck.debug=2", "agent.debug=2", "monitord.rotate_log=0", "analysisd.debug=2",
                         "cyware_modules.debug=2", "cyware_database.interval=1", "cyware_db.commit_time=2",
                         "cyware_db.commit_time_max=3", "remoted.debug=2"]
    DEBUG_OPTIONS = {
        'active_response': {
        ...
        'gcloud': {
            'manager': ['analysisd.debug=2', 'cyware_modules.debug=2', 'monitord.rotate_log=0']
        },
        'logtest': {
            'manager': ['analysisd.debug=2']
        },
        'remoted': {
            'manager': ['remoted.debug=2', 'cyware_database.interval=1', 'cyware_db.commit_time=2',
                        'cyware_db.commit_time_max=3', 'monitord.rotate_log=0']
        },
        'vulnerability_detector': {
            'manager': ['cyware_modules.debug=2', 'monitord.rotate_log=0']
        },
        'cyware_db': {
            'manager': ['cyware_modules.debug=2', 'monitord.rotate_log=0']
        },
        'wpk': {
            'manager': ['cyware_modules.debug=2'],
            'agent': {
                'generic': ['cyware_modules.debug=2'],
    ...
    def __init__(self, tests, ansible_inventory_path, qa_ctl_configuration, qa_framework_path=None):
        self.qa_framework_path = qa_framework_path if qa_framework_path is not None else \
                                                     os.path.join(gettempdir(), 'cyware_qa_ctl', 'cyware-qa')
        self.ansible_inventory_path = ansible_inventory_path
        self.qa_ctl_configuration = qa_ctl_configuration
        self.tests = tests

    def __set_local_internal_options(self, hosts, modules, component, system, cyware_install_path, ansible_admin_user):
        """Private method that set the local internal options in the hosts passed by parameter

            Args:
                hosts (list(str)): list of hosts aliases to index the dict attribute cyware_dir_paths and extract the
                                   cyware installation path.
                modules (list(str)): List of cyware modules to which the test belongs.
                component (str): Test cyware target (manager, agent).
                system (str): System where the test will be launched.
                cyware_install_path (str): Cyware installation directory p.e /var/ossec.
        ...
        playbook_file_path = os.path.join(gettempdir(), 'cyware_qa_ctl' f"{get_current_timestamp()}.yaml")

        if system == 'windows':
            local_internal_options_path = f"{cyware_install_path}\\local_internal_options.conf"
        else:
            local_internal_options_path = f"{cyware_install_path}/etc/local_internal_options.conf"
    ...
    def run(self):
        ...
        for test in self.tests:
            self.__set_local_internal_options(test.hosts, test.modules, test.component, test.system,
                                              test.cyware_install_path, test.ansible_admin_user)
