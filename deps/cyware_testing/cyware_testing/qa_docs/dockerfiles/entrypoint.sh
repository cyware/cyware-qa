...
git clone https://github.com/cyware/cyware-qa --depth=1 -b ${BRANCH} &> /dev/null
...
/usr/local/bin/qa-docs -p /tests/cyware-qa/tests --validate-parameters ${CMD}
...
echo "Running /usr/local/bin/qa-docs -p /tests/cyware-qa/tests ${CMD}"
/usr/local/bin/qa-docs -p /tests/cyware-qa/tests ${CMD}
