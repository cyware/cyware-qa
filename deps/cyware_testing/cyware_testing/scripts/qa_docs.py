...
from cyware_testing.qa_docs.lib.config import Config
from cyware_testing.qa_docs.lib.index_data import IndexData
from cyware_testing.qa_docs.lib.sanity import Sanity
from cyware_testing.qa_docs.lib import utils
from cyware_testing.qa_docs.doc_generator import DocGenerator
...
def validate_parameters(parameters, parser):
    ...
    if parameters.run_with_docker:
        ...
        if f"remotes/origin/{parameters.qa_branch}" not in branches:
            raise QAValueError(f"{parameters.qa_branch} not found in Cyware-QA repo.",
                               qadocs_logger.error)
    else:
