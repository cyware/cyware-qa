...
from cyware_testing.qa_ctl.deployment.qa_infraestructure import QAInfraestructure
from cyware_testing.qa_ctl.provisioning.qa_provisioning import QAProvisioning
from cyware_testing.qa_ctl.run_tests.qa_test_runner import QATestRunner
from cyware_testing.qa_ctl.run_tasks.qa_tasks_launcher import QATasksLauncher
from cyware_testing.qa_ctl.configuration.qa_ctl_configuration import QACTLConfiguration
...
from cyware_testing.tools.file import recursive_directory_creation
...
TASKS_KEY = 'tasks'
TEST_KEY = 'tests'
CYWARE_QA_FILES = os.path.join(gettempdir(), 'cyware_qa_ctl', 'cyware-qa')
...
def validate_configuration_data(configuration_data, qa_ctl_mode):
    ...
    if sys.platform == 'win32' and qa_ctl_mode == MANUAL_MODE:
        ...
        if not github_checks.branch_exists(configuration_data['config']['qa_ctl_launcher_branch'],
                                           repository=CYWARE_QA_REPO):
            raise QAValueError(f"{configuration_data['config']['qa_ctl_launcher_branch']} branch specified as "
                               'qa_ctl_launcher_branch  does not exist in Cyware QA repository.', qactl_logger.error,
                               QACTL_LOGGER)
...
def validate_test_module(type=None, component=None, suite=None, module=None):
    ...
    Args:
        ...
        module (str): Test module.
    """
    tests_path = os.path.join(CYWARE_QA_FILES, 'tests')
...
def set_parameters(parameters):
    ...
    Raises:
        QAValueError: If could not find a valid cyware tag in the first github tags page (It can happen if on the first
    ...
    try:
        parameters.version = parameters.version if parameters.version else github_checks.get_last_cyware_version()
    except QAValueError:
        raise QAValueError('The latest version of Cyware could not be obtained. Maybe there is no valid (non-rc) one at '
                           'https://github.com/cyware/cyware/tags. Try specifying the version manually using the '
...
def set_environment(parameters):
    ...
    Args:
        (argparse.Namespace): Object with the user parameters.
    """
    # Create the cyware_qa_ctl temporary folder
    recursive_directory_creation(os.path.join(gettempdir(), 'cyware_qa_ctl'))

    if parameters.run:
        # Download cyware-qa repository locally to run qa-docs tool and get the tests info
        local_actions.download_local_cyware_qa_repository(branch=parameters.qa_branch,
                                                         path=os.path.join(gettempdir(), 'cyware_qa_ctl'))
...
def validate_parameters(parameters):
    ...
    Raises:
        QAValueError: If parameters are incompatible, or version has not a valid format, or the specified cyware version
                      has not been released, or cyware QA branch does not exist (calculated from cyware_version).
    """
    ...
    if not github_checks.version_is_released(parameters.version):
        raise QAValueError(f"The cyware {parameters.version} version has not been released. Enter a right version.",
                           qactl_logger.error, QACTL_LOGGER)
    ...
    # Check if specified tests exist. Cyware-qa repository needs to be downloaded locally before.
    if parameters.run:
...
def get_script_parameters():
    ...
    description = \
        ...
        Description: qa-ctl is a tool for launching tests locally, automating the deployment, provisioning and testing
                     ...
                     You can find more information in https://github.com/cyware/cyware-qa/wiki/QACTL-tool
        '''
    ...
    parser.add_argument('--qa-branch', type=str, action='store', required=False, dest='qa_branch',
                        help='Set a custom cyware-qa branch to use in the run and provisioning. This '
...
def main():
    ...
    if qa_ctl_mode == AUTOMATIC_MODE:
        ...
        config_generator = QACTLConfigGenerator(modules_data, arguments.version, arguments.qa_branch,
                                                CYWARE_QA_FILES, arguments.operating_systems)
