...
from cyware_testing.tools import CYWARE_PATH, ACTIVE_RESPONSE_SOCKET_PATH
from cyware_testing.tools.utils import retry

request_socket = path.join(CYWARE_PATH, 'queue', 'sockets', 'request')
request_protocol = "tcp"


class CywareSocket:
    def __init__(self, socket_file=request_socket):
        """Encapsulate cyware-socket communication (header with message size)
    ...
    def send(self, msg, response_size=4):
        """Send and receive data to cyware-socket (header with message size)
        ...
        try:
            cyware_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            encoded_msg = msg_json.encode('utf-8')
            request_msg = struct.pack("<I", len(encoded_msg)) + encoded_msg

            connection(cyware_socket, self.file)
            send_msg(cyware_socket, request_msg)
            response = recv_response(cyware_socket, response_size)
            cyware_socket.close()
...
def send_request(msg_request, response_size=100, cyware_socket=request_socket):
