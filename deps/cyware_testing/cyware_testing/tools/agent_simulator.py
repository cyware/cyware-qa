...
import cyware_testing.data.syscollector as syscollector
import cyware_testing.data.winevt as winevt
import cyware_testing.cyware_db as wdb
from cyware_testing import TCP
from cyware_testing import is_udp, is_tcp
from cyware_testing.tools.monitoring import cyware_unpack, Queue
from cyware_testing.tools.remoted_sim import Cipher
from cyware_testing.tools.utils import retry, get_random_ip, get_random_string
...
class Agent:
    ...
    @staticmethod
    def cyware_padding(compressed_event):
        """Add the Cyware custom padding to each event sent.
        Args:
            compressed_event (bytes): Compressed event with zlib.
        Returns:
            bytes: Padded event.
        Examples:
            >>> cyware_padding(b'x\\x9c\\x15\\xc7\\xc9\\r\\x00 \\x08\\x04\\xc0\\x96\\\\\\x94\\xcbn0H\\x03\\xda\\x7f
    ...
    def create_event(self, message):
        ...
        # Padding
        padded_event = self.cyware_padding(compressed_event)
        # Encrypt
