...
if sys.platform == 'win32':
    ...
    CYWARE_CONF = os.path.join(CYWARE_PATH, 'ossec.conf')
    CYWARE_LOCAL_INTERNAL_OPTIONS = os.path.join(CYWARE_PATH, 'local_internal_options.conf')
    CYWARE_SOURCES = os.path.join('/', 'cyware')
    AGENT_CONF = os.path.join(CYWARE_PATH, 'shared', 'agent.conf')
    LOG_FILE_PATH = os.path.join(CYWARE_PATH, 'ossec.log')
    CLIENT_KEYS_PATH = os.path.join(CYWARE_PATH, 'client.keys')
    PREFIX = os.path.join('c:', os.sep)
    GEN_OSSEC = None
    CYWARE_API_CONF = None
    CYWARE_SECURITY_CONF = None
    API_LOG_FILE_PATH = None
    API_JSON_LOG_FILE_PATH = None
    API_LOG_FOLDER = None
    AGENT_STATISTICS_FILE = os.path.join(CYWARE_PATH, 'cyware-agent.state')
    LOGCOLLECTOR_STATISTICS_FILE = os.path.join(CYWARE_PATH, 'cyware-logcollector.state')
    REMOTE_STATISTICS_FILE = None
    ANALYSIS_STATISTICS_FILE = None
    UPGRADE_PATH = os.path.join(CYWARE_PATH, 'upgrade')
    AGENT_AUTH_BINARY_PATH = os.path.join(CYWARE_PATH, 'agent-auth.exe')
    ANALYSISD_BINARY_PATH = None
    HOSTS_FILE_PATH = os.path.join("C:", os.sep, "Windows", "System32", "drivers", "etc", "hosts")
    GLOBAL_DB_PATH = None
    CYWARE_UNIX_USER = 'cyware'
    CYWARE_UNIX_GROUP = 'cyware'
    GLOBAL_DB_PATH = os.path.join(CYWARE_PATH, 'queue', 'db', 'global.db')
    ARCHIVES_LOG_FILE_PATH = os.path.join(CYWARE_PATH, 'logs', 'archives', 'archives.log')
    ACTIVE_RESPONSE_BINARY_PATH = os.path.join(CYWARE_PATH, 'active-response', 'bin')
else:
    CYWARE_SOURCES = os.path.join('/', 'cyware')
    ...
    if sys.platform == 'darwin':
        ...
        GEN_OSSEC = os.path.join(CYWARE_SOURCES, 'gen_ossec.sh')
        PREFIX = os.sep

    CYWARE_CONF_RELATIVE = os.path.join('etc', 'ossec.conf')
    CYWARE_LOCAL_INTERNAL_OPTIONS = os.path.join(CYWARE_PATH, 'etc', 'local_internal_options.conf')
    CYWARE_CONF = os.path.join(CYWARE_PATH, CYWARE_CONF_RELATIVE)
    AGENT_CONF = os.path.join(CYWARE_PATH, 'etc', 'shared', 'agent.conf')
    CYWARE_API_CONF = os.path.join(CYWARE_PATH, 'api', 'configuration', 'api.yaml')
    CYWARE_SECURITY_CONF = os.path.join(CYWARE_PATH, 'api', 'configuration', 'security', 'security.yaml')
    LOG_FILE_PATH = os.path.join(CYWARE_PATH, 'logs', 'ossec.log')
    CLIENT_KEYS_PATH = os.path.join(CYWARE_PATH, 'etc', 'client.keys')
    API_LOG_FILE_PATH = os.path.join(CYWARE_PATH, 'logs', 'api.log')
    API_JSON_LOG_FILE_PATH = os.path.join(CYWARE_PATH, 'logs', 'api.json')
    API_LOG_FOLDER = os.path.join(CYWARE_PATH, 'logs', 'api')
    ARCHIVES_LOG_FILE_PATH = os.path.join(CYWARE_PATH, 'logs', 'archives', 'archives.log')
    AGENT_STATISTICS_FILE = os.path.join(CYWARE_PATH, 'var', 'run', 'cyware-agentd.state')
    LOGCOLLECTOR_STATISTICS_FILE = os.path.join(CYWARE_PATH, 'var', 'run', 'cyware-logcollector.state')
    LOGCOLLECTOR_FILE_STATUS_PATH = os.path.join(CYWARE_PATH, 'queue', 'logcollector', 'file_status.json')
    REMOTE_STATISTICS_FILE = os.path.join(CYWARE_PATH, 'var', 'run', 'cyware-remoted.state')
    ANALYSIS_STATISTICS_FILE = os.path.join(CYWARE_PATH, 'var', 'run', 'cyware-analysisd.state')
    UPGRADE_PATH = os.path.join(CYWARE_PATH, 'var', 'upgrade')
    PYTHON_PATH = os.path.join(CYWARE_PATH, 'framework', 'python')
    AGENT_AUTH_BINARY_PATH = os.path.join(CYWARE_PATH, 'bin', 'agent-auth')
    ANALYSISD_BINARY_PATH = os.path.join(CYWARE_PATH, 'bin', 'cyware-analysisd')
    ACTIVE_RESPONSE_BINARY_PATH = os.path.join(CYWARE_PATH, 'active-response', 'bin')
    AGENT_GROUPS_BINARY_PATH = os.path.join(CYWARE_PATH, 'bin', 'agent_groups')
    ...
    try:
        ...
        CYWARE_UID = pwd.getpwnam(CYWARE_UNIX_USER).pw_uid
        CYWARE_GID = grp.getgrnam(CYWARE_UNIX_GROUP).gr_gid
...
def get_version():
    ...
    if platform.system() in ['Windows', 'win32']:
        with open(os.path.join(CYWARE_PATH, 'VERSION'), 'r') as f:
            version = f.read()
            return version[:version.rfind('\n')]

    else:  # Linux, sunos5, darwin, aix...
        return subprocess.check_output([
          f"{CYWARE_PATH}/bin/cyware-control", "info", "-v"
...
def get_service():
    if platform.system() in ['Windows', 'win32']:
        ...
        service = subprocess.check_output([
          f"{CYWARE_PATH}/bin/cyware-control", "info", "-t"
        ], stderr=subprocess.PIPE).decode('utf-8').strip()

    return 'cyware-manager' if service == 'server' else 'cyware-agent'


_data_path = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'data')

CUSTOM_RULES_PATH = os.path.join(CYWARE_PATH, 'etc', 'rules')
LOCAL_RULES_PATH = os.path.join(CUSTOM_RULES_PATH, 'local_rules.xml')
LOCAL_DECODERS_PATH = os.path.join(CYWARE_PATH, 'etc', 'decoders', 'local_decoder.xml')
...
CYWARE_LOGS_PATH = os.path.join(CYWARE_PATH, 'logs')
ALERT_FILE_PATH = os.path.join(CYWARE_LOGS_PATH, 'alerts', 'alerts.json')
ALERT_LOGS_PATH = os.path.join(CYWARE_LOGS_PATH, 'alerts', 'alerts.log')
CLUSTER_LOGS_PATH = os.path.join(CYWARE_LOGS_PATH, 'cluster.log')
QUEUE_SOCKETS_PATH = os.path.join(CYWARE_PATH, 'queue', 'sockets')
QUEUE_ALERTS_PATH = os.path.join(CYWARE_PATH, 'queue', 'alerts')
QUEUE_DB_PATH = os.path.join(CYWARE_PATH, 'queue', 'db')
CLUSTER_SOCKET_PATH = os.path.join(CYWARE_PATH, 'queue', 'cluster')
...
GCLOUD_LOCAL_INTERNAL_OPTIONS = {'analysisd.debug': '2',
                                 'cyware_modules.debug': '2'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
LOGTEST_LOCAL_INTERNAL_OPTIONS = {'analysisd.debug': '2'}
REMOTED_LOCAL_INTERNAL_OPTIONS = {'remoted.debug': '2', 'cyware_database.interval': '2', 'cyware_db.commit_time': '2',
                                  'cyware_db.commit_time_max': '3'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
VD_LOCAL_INTERNAL_OPTIONS = {'cyware_modules.debug': '2'}.update(DISABLE_MONITORD_ROTATE_LOG_OPTION)
WPK_LOCAL_INTERNAL_OPTIONS = {'cyware_modules.debug': '2'}
