...
from cyware_testing.tools import CYWARE_CONF, CYWARE_API_CONF, API_LOG_FILE_PATH, CYWARE_LOCAL_INTERNAL_OPTIONS
from cyware_testing.tools.configuration import set_section_cyware_conf
...
class HostManager:
    ...
    def move_file(self, host: str, src_path: str, dest_path: str = '/var/ossec/etc/ossec.conf', check: bool = False):
        ...
        """
        self.get_host(host).ansible("copy", f"src={src_path} dest={dest_path} owner=cyware group=cyware mode=0775",
                                    check=check)
    ...
    def control_service(self, host: str, service: str = 'cyware', state: str = "started", check: bool = False):
        ...
        """
        if service == 'cyware':
            service = 'cyware-agent' if 'agent' in host else 'cyware-manager'
    ...
    def apply_config(self, config_yml_path: str, dest_path: str = CYWARE_CONF, clear_files: list = None,
        ...
        for host, payload in config.items():
            ...
            parse_configurations[host] = set_section_cyware_conf(sections=payload['sections'],
    ...
    def apply_api_config(self, api_config: str or dict = None, host_list: list = None, dest_path: str = CYWARE_API_CONF,
        ...
        for host in host_list:
            self.control_service(host=host, service='cyware-manager', state='restarted')
            if clear_log:
                self.clear_file(host=host, file_path=API_LOG_FILE_PATH)

    def get_api_token(self, host, user='cyware', password='cyware', auth_context=None, port=55000, check=False):
        """Return an API token for the specified user.

        Args:
            host (str): Hostname.
            user (str, optional): API username. Default `cyware`
            password (str, optional): API password. Default `cyware`
    ...
    def configure_local_internal_options(self, local_internal_options: dict):
        ...
        for target_host in local_internal_options:
            ...
            backup_local_internal_options = self.get_file_content(target_host, CYWARE_LOCAL_INTERNAL_OPTIONS)
            ...
            self.modify_file_content(target_host, CYWARE_LOCAL_INTERNAL_OPTIONS, replace)
