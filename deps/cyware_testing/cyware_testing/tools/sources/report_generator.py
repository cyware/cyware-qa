...
class LogAnalyzer:
    """This class group several statics methods to gather specific information from Cyware logs."""
    ...
    def keep_alive_log_parser(log_files):
        ...
        """
        keep_alive_regex = '(\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}) cyware\-remoted.* inserting ' + \
                           '\'(.*)\|(.*)\|(.*)\|(.*)\|(.* \[.*\].*)\n(.*)\n.*"_agent_ip":(\S+)'
...
class StatisticsAnalyzer:
    """This class group several statics methods to gather specific information from Cyware statistics."""
...
class CywareStatisticsAnalyzer():
    """This class group several statics methods to gather specific information from Cyware statistics."""
    @staticmethod
    def analyze_agentd_statistics(agentd_statistics_files):
        """Create a report for cyware-agentd daemon.
    ...
    def analyze_remoted_statistics(remoted_statistics_files):
        """Create a report for cyware-remoted daemon.
...
class ReportGenerator:
    ...
    def __init__(self, artifact_path):
        self.daemons_manager = ['cyware-modulesd', 'cyware-monitord', 'cyware-remoted', 'cyware-authd',
                                'cyware-db', 'cyware-syscheckd', 'cyware-analysisd']

        self.daemons_agent = ['cyware-modulesd', 'cyware-logcollector', 'cyware-syscheckd',
                              'cyware-agentd', 'cyware-execd']
    ...
    def get_instances_artifacts(self, component, hosts_regex=".*"):
        ...
        Args:
            component (str): Cyware installation type (agents/managers/all).
    ...
    def agentd_report(self, component='agents', hosts_regex='.*'):
        """Generate cyware-agentd daemon report, combining metric and statistics.

        Args:
            component (str): Cyware installation type (agents/managers/all).
            hosts_regex (str): Regex to filter by hostname.
        """
        agentd_report = CywareStatisticsAnalyzer.analyze_agentd_statistics(self.get_instances_statistics('cyware-agentd',
                                                                          component,
                                                                          hosts_regex))
        return agentd_report

    def remoted_report(self, component='managers', hosts_regex='.*'):
        """Generate cyware-remoted daemon report, combining metric and statistics.

        Args:
            component (str): Cyware installation type (agents/managers/all).
            hosts_regex (str): Regex to filter by hostname.
        """
        remoted_report = CywareStatisticsAnalyzer.analyze_remoted_statistics(self.get_instances_statistics(
                                                                                                        'cyware-remoted',
                                                                                                        component,
                                                                                                        hosts_regex))
    ...
    def metric_report(self, component='manager', hosts_regex='.*'):
        """Generate all Cyware metrics report.
    ...
    def make_report(self):
        ...
        report['managers'] = LogAnalyzer.get_error_logs_hosts(log_dict=self.get_instances_logs(log='all',
                                                                                               component='managers'))
        try:
            report['agents']['cyware-agentd'] = self.agentd_report()
        except Exception as e:
            unexpected_error = 'Unexpected error calculating agentd statistics'
            logging.error(unexpected_error)
            logging.error(e)
            report['agents']['cyware-agentd'] = {'ERROR': unexpected_error}

        try:
            report['managers']['cyware-remoted'] = self.remoted_report()
        except Exception as e:
            unexpected_error = 'Unexpected error calculating remoted statistics'
            logging.error(unexpected_error)
            logging.error(e)
            report['managers']['cyware-remoted'] = {'ERROR': unexpected_error}
