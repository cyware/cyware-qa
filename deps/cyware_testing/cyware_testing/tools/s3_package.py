...
PROD_BUCKET = 'packages.cyware.khulnasoft.com'
DEV_BUCKET = 'packages-dev.cyware.khulnasoft.com'
...
def get_s3_package_url(repository, target, version, revision, system, architecture, install_dir='/var', short_url=True):
    ...
    Args:
        ...
        target (string): Type of the cyware installation package.
        ...
        install_dir (string): Cyware installation directory
...
def get_repository_url(target, version, system, revision, repository, architecture, short_url=True):
    ...
    system_path = {
       SYSTEMS['rpm']: '/yum',
       SYSTEMS['deb']: f"/apt/pool/main/w/cyware-{target}",
...
def get_package_name(target, version, system, revision, repository, architecture):
    ...
    if system == SYSTEMS['rpm']:
        ...
        package_name = f"cyware_agent_v{version}{revision_section}_linux_x86_64.wpk"
    elif system == SYSTEMS['wpk-windows']:
        revision_section = f"-{revision}" if revision != '1' else ''
        package_name = f"cyware_agent_v{version}{revision_section}_windows.wpk"
    else:
...
def get_production_package_url(cyware_target, os_system, cyware_version):
    ...
    Args:
        ...
        cyware_version (str): Cyware version (e.g 4.2.5)
    ...
    return get_s3_package_url('live', cyware_target, cyware_version, '1', package_system, architecture)


def get_last_production_package_url(cyware_target, os_system):
    ...
    """
    last_cyware_version = github_checks.get_last_cyware_version()

    return get_production_package_url(cyware_target, os_system, last_cyware_version)
