...
class CustomLogger:
    def __init__(self, name, file_path='/tmp/cyware_api_simulator.log', foreground=False, tag='Main',
                 level=logging.INFO):
...
class APISimulator:
    def __init__(self, host, port, protocol='https', frequency=60, user='cyware-wui', password='cyware-wui',
                 external_logger=None, request_percentage=0, request_template=None):
        self.host = host
        self.port = port
        self.protocol = protocol
        self.frequency = frequency
        self.logger = external_logger if external_logger else logging.getLogger('cyware-api-requester')
        self.user = user
