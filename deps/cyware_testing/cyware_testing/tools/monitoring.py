...
REMOTED_DETECTOR_PREFIX = r'.*cyware-remoted.*'
LOG_COLLECTOR_DETECTOR_PREFIX = r'.*cyware-logcollector.*'
AGENT_DETECTOR_PREFIX = r'.*cyware-agent.*' if sys.platform == 'win32' else r'.*cyware-agentd.*'
WINDOWS_AGENT_DETECTOR_PREFIX = r'.*cyware-agent.*'
AUTHD_DETECTOR_PREFIX = r'.*cyware-authd.*'
MODULESD_DETECTOR_PREFIX = r'.*cyware-modulesd.*'
CYWARE_DB_PREFIX = r'.*cyware-db.*'

DEFAULT_POLL_FILE_TIME = 1
DEFAULT_WAIT_FILE_TIMEOUT = 30


def cyware_unpack(data, format_: str = "<I"):
...
def cyware_pack(data, format_: str = "<I"):
...
def make_callback(pattern, prefix="cyware", escape=False):
    """
...
class SocketController:
    ...
    def send(self, message, size=False):
        ...
        try:
            msg_bytes = cyware_pack(len(msg_bytes)) + msg_bytes if size else msg_bytes
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def unix_forward(self, data):
        ...
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as forwarded_sock:
            ...
            forwarded_sock.sendall(cyware_pack(len(data)) + data)

            # Receive data from the server and shut down
            size = cyware_unpack(self.recvall_size(forwarded_sock, 4, socket.MSG_WAITALL))
    ...
    def default_cyware_handler(self):
        ...
        while not self.server.mitm.event.is_set():
            ...
            self.request.sendall(cyware_pack(len(response)) + response)
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam('cyware').pw_uid
            gid = grp.getgrnam('cyware').gr_gid
