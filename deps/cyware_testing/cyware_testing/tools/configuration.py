...
from cyware_testing import global_parameters, logger, VALID_FIM_MODES, OS_EXCLUDED_FROM_RT_WD
from cyware_testing.tools import CYWARE_PATH, GEN_OSSEC, CYWARE_CONF, PREFIX, CYWARE_LOCAL_INTERNAL_OPTIONS, AGENT_CONF, \
                                LOCAL_RULES_PATH
...
def set_cyware_conf(cyware_conf: List[str]):
    """
    Set up Cyware configuration. Cyware will be restarted to apply it.

    Args:
        cyware_conf (ET.ElementTree): ElementTree with a custom Cyware configuration.
    """
    write_cyware_conf(cyware_conf)
    print("Restarting Cyware...")
    command = os.path.join(CYWARE_PATH, 'bin/cyware-control')
    arguments = ['restart']
    check_call([command] + arguments, stdout=DEVNULL, stderr=DEVNULL)


def generate_cyware_conf(args: List = None) -> ET.ElementTree:
    """
    Generate a configuration file for Cyware.

    Args:
        args (list, optional): Arguments to generate ossec.conf (install_type, distribuition, version). Default `None`

    Returns:
        (ET.ElementTree): New Cyware configuration generated from 'gen_ossec.sh'.
    """
    gen_ossec_args = args if args else ['conf', 'manager', 'rhel', '7']
    cyware_config = check_output([GEN_OSSEC] + gen_ossec_args).decode(encoding='utf-8', errors='ignore')

    return ET.ElementTree(ET.fromstring(cyware_config))
...
def write_cyware_conf(cyware_conf: List[str]):
    """
    ...
    """
    with open(CYWARE_CONF, 'w') as f:
        f.writelines(cyware_conf)
...
def write_security_conf(path: str, security_conf: dict):
    ...
    if not os.path.exists(path):
        from cyware_testing.tools import CYWARE_UID, CYWARE_GID

        open(path, mode='w').close()
        os.chown(uid=CYWARE_UID, gid=CYWARE_GID, path=path)
    write_api_conf(path, security_conf)


def set_section_cyware_conf(sections, template=None):
    """
    ...
    for section in sections:
        ...
        section_conf = find_module_config(cyware_conf, section['section'], attributes)
        # Create section if it does not exist, clean otherwise
        if not section_conf:
            section_conf = ET.SubElement(cyware_conf.getroot(), section['section'])
            section_conf.text = '\n    '
            section_conf.tail = '\n\n  '
        else:
...
def load_cyware_configurations(yaml_file_path: str, test_name: str, params: list = None, metadata: list = None) -> Any:
    r"""
...
def get_cyware_local_internal_options() -> List[str]:
    """Get current `internal_options.conf` file content.

    Returns
        List of str: A list containing all the lines of the `ossec.conf` file.
    """
    with open(CYWARE_LOCAL_INTERNAL_OPTIONS) as f:
        lines = f.readlines()
    return lines


def set_cyware_local_internal_options(cyware_local_internal_options: List[str]):
    """Set up Cyware `local_internal_options.conf` file content.

    Returns
        List of str: A list containing all the lines of the `local_interal_options.conf` file.
    """
    with open(CYWARE_LOCAL_INTERNAL_OPTIONS, 'w') as f:
        f.writelines(cyware_local_internal_options)


def add_cyware_local_internal_options(cyware_local_internal_options_dict):
    """Add new local internal options to the current configuration.

    Args:
        cyware_local_internal_options_dict (List of str): A list containing local internal options to add.
    """
    local_internal_options_str = create_local_internal_options(cyware_local_internal_options_dict)
    with open(CYWARE_LOCAL_INTERNAL_OPTIONS, 'a') as f:
...
def create_local_internal_options(dict_local_internal_options):
    ...
    for key, value in dict_local_internal_options.items():
        cyware_local_internal_options += f"{str(key)}={str(value)}\n"
    return cyware_local_internal_options
...
def write_cyware_local_rules(local_rules: List[str]):
    """
...
def get_minimal_configuration():
    ...
    Returns:
        str: Cyware minimal configuration data.
    """
