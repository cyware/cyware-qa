...
def version_is_released(version, organization='cyware', repository='cyware'):
...
def branch_exists(branch_name, organization='cyware', repository='cyware'):
...
def get_last_cyware_version():
    """Get the last Cyware version that has been tagged, regardless of possible release candidate tags.
    ...
    Returns:
        str: Last Cyware tag (no rc).
    """
    url = 'https://github.com/cyware/cyware/tags'
    req = requests.get(url)
    last_tags = re.findall(r'<a href="/cyware/cyware/releases/tag/v\d.\d.\d">', req.text)

    try:
        last_cyware_version = re.compile(r'<a href="/cyware/cyware/releases/tag/v(\d.\d.\d)').search(last_tags[0]).group(1)
    except IndexError:
        raise QAValueError(f"Could not find a valid cyware tag in {url}")

    return last_cyware_version
