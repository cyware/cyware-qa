...
from cyware_testing.tools import CYWARE_PATH, get_service, CYWARE_SOCKETS, QUEUE_DB_PATH, CYWARE_OPTIONAL_SOCKETS
from cyware_testing.tools.configuration import write_cyware_conf
from cyware_testing.modules import CYWARE_SERVICES_START, CYWARE_SERVICES_STOP


def restart_cyware_daemon(daemon):
    """Restarts a Cyware daemon.
    ...
    daemon_path = os.path.join(CYWARE_PATH, 'bin')
    subprocess.check_call([f'{daemon_path}/{daemon}'])


def restart_cyware_with_new_conf(new_conf, daemon='cyware-syscheckd'):
    """Restart Cyware service applying a new ossec.conf.

    Args:
        new_conf ( ET.ElementTree) : New config file.
        daemon (str, optional): Daemon to restart when applying the configuration.
    """
    write_cyware_conf(new_conf)
...
def delete_sockets(path=None):
    ...
    try:
        if path is None:
            path = os.path.join(CYWARE_PATH, 'queue', 'sockets')
            for file in os.listdir(path):
                os.remove(os.path.join(path, file))
            if os.path.exists(os.path.join(CYWARE_PATH, 'queue', 'db', 'wdb')):
                os.remove(os.path.join(CYWARE_PATH, 'queue', 'db', 'wdb'))
            if os.path.exists(os.path.join(CYWARE_PATH, 'queue', 'cluster', 'c-internal.sock')):
                os.remove(os.path.join(CYWARE_PATH, 'queue', 'cluster', 'c-internal.sock'))
        else:
            for item in path:
                os.remove(item)
    ...
    if sys.platform == 'win32':
        if action == 'restart':
            ...
            for _ in range(error_109_windows_retry):
                command = subprocess.run(["net", action, "CywareSvc"], stderr=subprocess.PIPE)
        ...
        if daemon is None:
            if sys.platform == 'darwin' or sys.platform == 'sunos5':
                result = subprocess.run([f'{CYWARE_PATH}/bin/cyware-control', action]).returncode
            else:
            ...
            if action == 'restart':
                ...
                for proc in psutil.process_iter():
                    try:
                        if daemon in ['cyware-clusterd', 'cyware-apid']:
                            for file in os.listdir(f'{CYWARE_PATH}/var/run'):
                                if daemon in file:
                                    pid = file.split("-")
                                    pid = pid[2][0:-4]
                                    if pid == str(proc.pid):
                ...
                delete_sockets(CYWARE_SOCKETS[daemon])
            else:
                daemon_path = os.path.join(CYWARE_PATH, 'bin')
...
def restart_cyware_function():
    """Restarts Cyware."""
    control_service(CYWARE_SERVICES_STOP)
    control_service(CYWARE_SERVICES_START)
...
def check_daemon_status(target_daemon=None, running_condition=True, timeout=10, extra_sockets=[]):
    ...
    while elapsed_time < timeout and not condition_met:
        if sys.platform == 'win32':
            condition_met = check_if_process_is_running('cyware-agent.exe') == running_condition
        else:
            control_status_output = subprocess.run([f'{CYWARE_PATH}/bin/cyware-control', 'status'],
