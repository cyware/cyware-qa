...
def callback_detect_start_gcp(line):
    match = re.match(r'.*cyware-modulesd:gcp-pubsub.*INFO: Module started.$', line)
    if match:
        return line
    return None
...
def detect_gcp_start(file_monitor):
    """Detect module gcp-pubsub starts after restarting Cyware.
...
def callback_detect_all_gcp(line):
    match = re.match(r'.*cyware-modulesd:gcp-pubsub.*', line)
    if match:
        return line
    return None
...
def publish(id_project, name_topic, credentials, messages=None):
    if CYWARE_PATH in credentials:
        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "{}".format(credentials)
    else:
        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "{}/{}".format(CYWARE_PATH, credentials)
