...
from cyware_testing import UDP, TCP, ARCHIVES_LOG_PATH, LOG_FILE_PATH, QUEUE_SOCKETS_PATH, CYWARE_PATH
from cyware_testing.tools.file import bind_unix_socket, truncate_file
from cyware_testing.tools.monitoring import FileMonitor, make_callback, ManInTheMiddle, QueueMonitor, \
...
EXAMPLE_VALID_USER_LOG_EVENT = '2021-03-04T02:16:16.998693-05:00 centos-8 su - - [timeQuality tzKnown="1" ' \
                               'isSynced="0"] pam_unix(su:session): session opened for user cyware_qa by (uid=0)'
...
def new_agent_group(group_name=DEFAULT_TESTING_GROUP_NAME, configuration_file='agent.conf'):
    ...
    sb.run([f"{CYWARE_PATH}/bin/agent_groups", "-q", "-a", "-g", group_name])

    agent_conf_path = os.path.join(data_path, configuration_file)

    with open(f"{CYWARE_PATH}/etc/shared/{group_name}/agent.conf", "w") as agent_conf_file:
        with open(agent_conf_path, 'r') as configuration:
            agent_conf_file.write(configuration.read())


def remove_agent_group(group_name):
    sb.run([f"{CYWARE_PATH}/bin/agent_groups", "-q", "-r", "-g", group_name])


def add_agent_to_group(group_name, agent_id):
    sb.run([f"{CYWARE_PATH}/bin/agent_groups", "-q", "-a", "-i", agent_id, "-g", group_name])
...
def callback_invalid_value(option, value):
    ...
    Args:
        option (str): Cyware manager configuration option.
...
def callback_ignored_invalid_protocol(protocol):
    ...
    Args:
        protocol (str): Cyware manager protocol.
...
def callback_detect_remoted_started(port, protocol, connection_type="secure"):
    ...
    cyware-remoted logs if it has correctly started for each connection type, the port and the protocol in the ossec.log

    Args:
        port (int): port configured for cyware-remoted.
        protocol (str): protocol configured for cyware-remoted. It can be UDP, TCP or both options at the same time.
...
def send_syslog_message(message, port, protocol, manager_address="127.0.0.1"):
    """Send a message to the syslog server of cyware-remoted.
...
def create_archives_log_monitor():
    ...
    cyware_archives_log_monitor = FileMonitor(ARCHIVES_LOG_PATH)

    return cyware_archives_log_monitor
...
def check_syslog_event(cyware_archives_log_monitor, message, port, protocol, timeout=10):
    """Check if a syslog event is properly received by the manager.

    Args:
        cyware_archives_log_monitor (FileMonitor): FileMonitor object to monitor the archives.log.
    ...
    # Syslog events may contain a PRI header at the beginning of the message <1>. If cyware-remoted receives a message
    ...
    for msg in parsed_msg.split("\n"):
        detect_archives_log_event(archives_monitor=cyware_archives_log_monitor,
...
def check_remoted_log_event(cyware_log_monitor, callback_pattern, error_message='', update_position=False,
                            timeout=REMOTED_GLOBAL_TIMEOUT):
    """Allow to monitor the ossec.log file and search for a remoted event.

    Args:
        cyware_log_monitor (FileMonitor): FileMonitor object to monitor the Cyware log.
...
def check_tcp_connection_established_log(cyware_log_monitor, update_position=False, ip_address='127.0.0.1'):
    ...
    check_remoted_log_event(cyware_log_monitor, callback_pattern, error_message, update_position)


def wait_to_remoted_key_update(cyware_log_monitor):
    ...
    check_remoted_log_event(cyware_log_monitor, callback_pattern, error_message, timeout=20)


def wait_to_remoted_update_groups(cyware_log_monitor):
    ...
    check_remoted_log_event(cyware_log_monitor, callback_pattern, error_message, timeout=SYNC_FILES_TIMEOUT)


def send_agent_event(cyware_log_monitor, message=EXAMPLE_MESSAGE_EVENT, protocol=TCP, manager_address='127.0.0.1',
    ...
    wait_to_remoted_key_update(cyware_log_monitor)
...
def check_queue_socket_event(raw_events=EXAMPLE_MESSAGE_PATTERN, timeout=30, update_position=False):
    ...
    try:
        ...
        mitm.shutdown()
        control_service('start', daemon='cyware-analysisd')
...
def check_push_shared_config(agent, sender, injector=None):
    ...
    try:
        ...
        cyware_log_monitor.start(timeout=REMOTED_GLOBAL_TIMEOUT, callback=log_callback,
        ...
        wait_to_remoted_update_groups(cyware_log_monitor)

        keep_alive_agent = multiprocessing.Process(target=keep_alive_until_group_configuration_sent,
                                                   args=(sender,))
        keep_alive_agent.start()

        log_callback = make_callback(pattern=r".*End sending file '.+' to agent '\d+'\.", prefix=r'.*cyware-remoted.*')
