...
from cyware_testing.tools import GLOBAL_DB_PATH, CYWARE_DB_SOCKET_PATH
from cyware_testing.tools.monitoring import cyware_pack, cyware_unpack
from cyware_testing.tools.services import control_service


def callback_cywaredb_response(item):
...
def mock_db(func):
    """Decorator used in any function that needs to mock a cyware db

    This function will execute `func` after stopping cyware-modulesd and cyware-db. After that,
...
def run_query(db_query, db_path=GLOBAL_DB_PATH):
    """Method used to run sqlite queries on cyware databases
...
def query_wdb(command):
    ...
    try:
        sock.send(cyware_pack(len(command)) + command.encode())
